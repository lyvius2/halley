...
| P0 운영 기본값 | **완료** (설계 I296). 가입 기본 `false`; live DB 자격은 환경변수 없으면 기동 실패; admin 초기 비밀번호는 주면 로그에 안 남기고 **없어도 뜬다**(09-25 조정 — 첫 배포에서 강제가 걸려 되돌림). `SignUpDefaultClosedTest` · `AdminBootstrapTest` |
