...
> **최종 갱신**: 2026-09-01 · **구현 0~9 완료 · 구현 10만 남음**
>
> 처음에는 "구현 가능성 검토" 문서였습니다. 지금은 <b>돌고 있는 기능의 설계서</b>입니다 —
> 검토 단계의 서술이 남아 있으면 그건 <b>그때의 판단 기록</b>입니다.
> 결정 이력은 `docs/DESIGN.md` I127~I163.
...
| ~~정비사업 단계~~ | 지자체별 포털 | — | **접었습니다** — 전국 단일 API가 없다 (4-A.3) |
...
| 조건 | 강제 | 왜 |
|---|---|---|
| 실거래를 세는 지표가 **하나도 없음** | `UNCERTAIN` — LLM이 뭐라 하든 덮어쓴다 | 3건으로는 <b>누구도</b> 알 수 없다 |
...
> **첫 항목의 "실거래를 세는 지표"는 <b>실거래 추세</b>(3개월 창)와 <b>장기 추세</b>(12개월 창)입니다.**
> 각 지표가 이미 3건 미만이면 값을 내지 않으므로(4.1 · 4.6), <b>둘 중 하나라도 나왔다면 표본이 있었다</b>는 뜻입니다.
>
> 금리 국면과 용적률 여유는 **여기 들어가지 않습니다.** ECOS 통계와 건축물대장이라
> 아무리 잘 나와도 <b>이 매물의 실거래 표본</b>과는 무관합니다.
>
> 처음에는 <b>실거래 추세 하나만</b> 봤습니다. 그러면 장기 표본이 넉넉해도
> **최근 석 달이 한산하면 판단이 덮입니다** — <b>3개월 창이 얇은 것</b>과
> <b>실거래 자료가 없는 것</b>은 다른 얘기인데 둘을 같게 본 것이었습니다 (I151).
...
```json
{
  ...
  "codeDirection": "FLAT",      // ← 코드 예측. 모달의 참고 문구에만 쓴다
  "agreed": false,
  "confidence": "LOW",
  ...
}
...
같은 법정동·같은 달은 매물이 달라도 **국토부 응답이 동일**합니다. 그러니 캐시가 크게 듣습니다.

> **⚠ `reference_transaction`은 그 캐시가 아닙니다.** 이 문서 초판에 "이미 받은 달은
> `reference_transaction`에서 건너뜁니다"라고 썼는데 <b>틀렸습니다.</b> 실제 코드는 이렇습니다.

```java
// ReferenceTransactionService — 저장 전에 걸러지고 잘린다
.filter(trade -> matches(property, trade))   // 같은 단지 · 면적 ±15%
.limit(MAX_SAVED)                            // 50건
.map(trade -> referenceTransactionRepository.save(new ReferenceTransaction(
        null, propertyId, ...)))             // 키가 propertyId 다
```

| | `reference_transaction` | 전망에 필요한 것 |
|---|---|---|
| 키 | **매물 번호** | **(법정동코드, 년월)** |
| 내용 | 단지·면적이 맞는 것만 | **원본 응답 전부** |
| 개수 | 50건까지 | 제한 없음 |
| 재사용 | 그 매물에서만 | **같은 법정동의 모든 매물** |

그대로 두면 같은 법정동의 두 번째 매물도 **60개월을 처음부터 다시 받습니다.**

#### 그래서 월별 캐시를 따로 둡니다

`(법정동코드, 년월)`을 키로 **원본 응답을 그대로** 보관합니다 — 필터도 50건 제한도 없이.

> **과거 달은 바뀌지 않습니다.** 국토부 신고 지연을 감안해 <b>최근 3개월만 다시 받고</b>,
> 그 이전은 TTL 없이 영구 보관해도 됩니다. 60개월 중 57개월이 한 번 받으면 끝입니다.

```
첫 매물        60회 호출  →  3~10초
같은 법정동 2번째   0~3회   →  1초 미만
```

이 캐시를 만드는 것이 **구현 1**입니다(7장).
...
**모달에서 한 문장으로** 알립니다(5.2). 감춰 두면 사용자는 그 판단이 얼마나
단단한지 알 수 없습니다.

> **목록의 화살표는 건드리지 않습니다.** 색을 흐리는 안을 검토했다가 접었습니다 —
> 코드 예측의 임계값이 임의의 값이라, 그것 때문에 화살표가 흐려지면
> <b>없는 불확실성을 만들어 냅니다.</b>

**둘 다 저장합니다.** 나중에 실제 결과와 맞춰 보면(10단계) <b>어느 쪽이 더 맞았는지</b>
알 수 있습니다 — 이 데이터가 없으면 그 검증 자체가 불가능합니다.

> **코드 예측의 임계값은 여전히 임의입니다.** 결론도 화살표도 정하지 않고
> <b>모달의 한 문장에만</b> 영향을 주므로 위험이 작습니다. 그래도 `regulation_param`에
> 두고 조정 가능하게 합니다 — 10단계 검증에서 조정할 거리입니다.
...
**V-World 토지이용계획에는 단계가 없습니다.** 별도 출처가 필요한데 — <b>여기서 막힙니다.</b>
...
> **전국 단일 API가 없어 접었습니다.** 서울 정비사업 정보몽땅은 공식 오픈 API 여부가
> 불확실하고, 서울 밖은 지자체마다 흩어져 있습니다. <b>한 지역만 되는 연동</b>은
> 나머지 지역에서 "확인 필요"만 남깁니다.
>
> **대신 이렇게 합니다.** 토지이용계획의 `정비구역` 표시(이미 받고 있습니다)로
> <b>"이 매물은 정비사업 진행 상황이 가격을 좌우합니다"</b>라고 말해 주는 선까지만 합니다.
> 단계를 모르면서 아는 척하지 않습니다.
>
> 다시 꺼낼 계기: 실제로 정비구역 매물을 여러 건 보게 됐을 때.
...
> **화살표는 오직 LLM 예측만 나타냅니다.** 코드 예측과 갈려도 <b>색을 흐리거나 모양을
> 바꾸지 않습니다.</b> 목록은 여러 매물을 견주는 자리라 신호가 하나여야 읽힙니다.
>
> 농담으로 일치 여부를 나타내는 안을 검토했다가 접었습니다 — 연한 검정(횡보 불일치)이
> '분석 중' 회색과 부딪히고, <b>채도 차이는 색각 이상에서 색상 차이보다 더 어렵습니다.</b>
> 그리고 코드 예측의 임계값은 임의의 값이라, 그것 때문에 화살표가 흐려지면
> <b>없는 불확실성을 만들어 냅니다.</b>
>
> **둘이 갈린 사실은 모달에서 문장으로 말합니다**(5.2).
...
#### 코드 예측과 갈렸을 때 — 문장으로 말합니다

목록의 화살표는 건드리지 않고, **모달에서 한 줄로** 알립니다.

| 상황 | 문구 |
|---|---|
| 갈림 | `AI 모델은 <b>상승</b>을 예측했지만 규칙 기반 계산은 <b>횡보</b>였음을 참고하십시오.` |
| 일치 | `규칙 기반 계산도 같은 방향입니다.` |
| 코드가 판단 못 함 | *(아무 말도 하지 않습니다)* |

> **경고가 아니라 참고입니다.** `⚠`가 아니라 `ℹ`를 씁니다 — 코드 예측의 임계값은
> 임의의 값이라, 갈렸다는 것이 <b>LLM이 틀렸다는 뜻은 아닙니다.</b>
> 다만 "이 판단이 얼마나 단단한가"의 재료로는 값어치가 있습니다.

> **일치할 때도 한 줄 남깁니다.** 아무 말이 없으면 사용자는 <b>비교를 안 한 것인지
> 일치한 것인지</b> 알 수 없습니다.
...
5. **틀린 전망을 남겨 둡니다.** 과거 전망과 실제 결과를 함께 보여 주면
   ...
   그래서 `price_forecast_history`에 <b>지금부터 쌓습니다</b>(I138). 판정 기준은
   나중에 바꿀 수 있지만 <b>지나간 시간은 되돌릴 수 없습니다.</b>

---

## 7. 구현 순서

> **'단계'라는 말이 두 곳에 쓰입니다.** 3-A의 <b>1·2·3단계</b>는 매물 등록 시
> <b>실행되는 시점</b>이고, 아래 번호는 <b>만드는 순서</b>입니다. 헷갈리지 않게
> 아래는 `구현 N`으로 부릅니다.
...
| **4** | 지표 추가 — 전세가율 · 금리 국면 · **용적률 여유** | **건축물대장** | 4.2~4.4 · 4-A.1·2 — 키는 재사용, 활용신청만 |
| **5** | **코드 예측기** (규칙 기반) | 없음 | 4.5 — 이중 예측의 한쪽 |
| **6** | **LLM 판단·해석** + 안전장치 | 없음 | 2.2 · 2.2-A |
| **7** | 저장 · 이벤트 · 폴링 | 없음 | I126 · 3-A.2 · 3-A.4 |
| **8** | **화면** — 화살표 · 모달 | 없음 | 5장 |
| **9** | 관련 기사 링크 목록 (점수 미반영) | 네이버 검색 | 4-B · I137 |
| 10 | 과거 전망 대비 실제 결과 | 없음 | <b>이력 저장만 먼저 넣었다</b>(I138) — 판정 기준은 표본을 보고 정한다 |
| ~~정비사업 단계~~ | **접었습니다** | — | 4-A.3 — 전국 단일 API가 없다 |

> **0~9는 전부 구현됐습니다** (I127~I148). 그 뒤 운영에서 드러난 것들이
> I149~I151 · I159에 있습니다 — 토큰 예산, 초당 제한, 안전장치가 취지보다 좁았던 것.
>
> **구현 10만 남았습니다.** 이력(I138)과 월간 재계산(I143)을 넣어 <b>재료는 쌓이기
> 시작했고</b>, 판정 기준 두 가지(무엇을 '맞았다'로 볼지 · 표본 하한)는
> <b>표본을 보고</b> 정합니다.

#### 그 뒤 늘어난 지표

설계 당시 요인은 넷이었는데 지금은 **여섯**입니다 — <b>장기 추세</b>와
<b>전고점 대비</b>가 더해졌습니다(I148). 60개월을 받아 놓고 12개월만 읽고 있었기 때문입니다.

**구현 0~8에서 새로 붙는 것은 건축물대장 하나뿐입니다.** 그것도 `MINISTRY_API_KEY`를
재사용하므로 <b>활용신청만 하면 됩니다.</b> 나머지는 이미 붙은 API와 받아 놓고 안 쓰던 데이터입니다.

#### 건축물대장을 구현 4에 넣은 이유

**그것 없이는 용도지역 지표가 반쪽입니다.**

```
없이:   "제3종일반주거, 상한 300%"            ← 여유가 얼마인지 모른다
있으면: "상한 300%, 현재 240% → 여유 60%p"    ← 재건축 여력이 보인다
```

"상한 300%"만으로는 아무 판단도 못 합니다. 그리고 **키가 이미 있어** 붙이는 비용이 작습니다.

#### 정비사업 단계를 접은 이유

값어치는 가장 크지만 **전국 단일 API가 없습니다.** 서울은 정비사업 정보몽땅이 있으나
공식 오픈 API 여부가 불확실하고, 서울 밖은 지자체마다 흩어져 있습니다.
<b>한 지역만 되는 연동</b>은 나머지 지역에서 "확인 필요"만 남깁니다.

> 다시 꺼낼 계기: 실제로 정비구역 매물을 여러 건 보게 됐을 때.
> 그전에는 토지이용계획의 `정비구역` 표시로 <b>"이 매물은 정비사업 진행 상황이
> 가격을 좌우합니다"</b>라고 말해 주는 선까지만 합니다(4-A.3).
...
application/port/out/external/BuildingLedgerPort.java       ← 신규
adapter/outbound/external/ministry/BuildingLedger{FeignClient,Adapter,FallbackFactory}
config/RegulationParamBootstrap.java                        용적률 상한 시드
docs/INTERFACE_MANUAL.md                                    5.11 · 5.13 [구현 예정] → 구현됨
```

| 지표 | 재료 | 상태 |
|---|---|---|
| 전세가율 | 국토부 **전월세** 오퍼레이션 | 키 재사용 (5.13) |
| 금리 국면 | ECOS 가계대출금리 12개월 기울기 | **어댑터 완성됨** (I116) |
| 용적률 여유 | 토지이용계획(상한) + **건축물대장**(현재) | 5.11 — <b>이번에 붙인다</b> |

#### 건축물대장

**`getBrRecapTitleInfo`(총괄표제부)를 씁니다.** 표제부(`getBrTitleInfo`)는 동마다 한 행이라
대지면적이 동 수만큼 중복되고, 그대로 더하면 용적률이 엉뚱해집니다. 총괄표제부가 없는
단지만 표제부로 내려갑니다. 자세한 규격은 `INTERFACE_MANUAL.md` 5.11.

```
용적률 여유 = 조례 상한(regulation_param) − vlRat(대장)
```

> **`platGbCd` 변환에 주의하십시오.** PNU의 산 여부는 `1`=대지·`2`=산인데
> 건축물대장은 `0`=대지·`1`=산입니다. 그대로 넘기면 엉뚱한 필지를 봅니다.

> **대장을 못 받으면 이 지표를 내지 않습니다.** 근사값으로 채우지 않습니다 —
> 재건축 사업성은 가장 크게 틀릴 수 있는 숫자입니다.
...
**검증**
- 각 지표 단위 테스트
- 전월세 API 실호출 1회로 응답 모양 확인
- **건축물대장 실호출 1회** — 5.11의 확인 항목 셋(오퍼레이션 존재 · `vlRat` 채워짐 · 총괄표제부 유무)
