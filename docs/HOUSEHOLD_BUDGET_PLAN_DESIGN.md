...
moving_cost
cleaning_cost
household_reserve_cost       -- 배송·조립·설치 예비비
other_initial_cost
parent_support
other_funds
...
추가 필요자금
= 총 필요자금 - (대출금 + 투입 가능 자산 + 부모 지원금 + 기타 자금)
```

대출금은 총 필요자금과 확보 자금에서 한 번만 반영한다. `주택 자기자금`은 매매가격·보증금에서
대출금을 뺀 참고 지표이며, 계약금과 잔금의 합계가 이 값과 다르면 화면에서 확인을 요청한다.
...
등록 매물을 처음 선택할 때 부대비용 입력값이 모두 비어 있으면 PoC 추정값을 함께 적용한다.
이미 입력한 부대비용은 덮어쓰지 않으며, 사용자가 수정한 값은 수기 입력으로 표시한다.
