...
| 최초 admin 비밀번호 | 만들어서 INFO 로그 | `HALLEY_ADMIN_INITIAL_PASSWORD` 로 받으면 <b>로그에 남기지 않는다</b>. live 는 이 변수를 강제한다 |

환경변수를 빠뜨린 배포가 곧 가입이 열리고 기본 자격으로 DB 에 붙는 배포였습니다.
이제 빠뜨리면 <b>뜨지 않습니다</b> — 조용히 위험하게 뜨는 것보다 낫습니다.

로컬은 그대로입니다. 비밀번호를 안 정하면 만들어서 로그에 적습니다(WARN) — 첫 부팅에서
읽을 다른 길이 없어서입니다. 다만 "운영이라면 환경변수를 두라"고 같이 적습니다.
