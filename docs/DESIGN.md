...
### I295. 세션 쿠키 인증인데 CSRF 가 꺼져 있었다 · **[확정 — 구현됨]**

`SecurityConfig` 가 `.csrf(disable)` 이었습니다. 이 서비스는 `JSESSIONID` 쿠키로 인증하고,
쿠키는 브라우저가 알아서 붙입니다. 다른 사이트의 문서가 우리 API 로 POST 를 던지면
<b>로그인한 사람의 권한으로</b> 실행됐습니다 — 매물 삭제·점수 수정·웹훅 변경까지.

#### 쿠키로 내리고 헤더로 되돌려 받는다

`CookieCsrfTokenRepository` 가 `XSRF-TOKEN` 쿠키를 내리고, 화면이 그 값을 `X-XSRF-TOKEN`
헤더로 되돌려 보냅니다. 다른 출처의 문서는 이 쿠키를 <b>읽을 수 없어</b> 헤더를 못 만듭니다.
그래서 이 쿠키만은 HttpOnly 가 아닙니다.

토큰은 <b>누가 읽어야</b> 쿠키에 적힙니다. 화면은 셸(`GET /`)만 받고 API 를 부르므로,
`CsrfCookieFilter` 가 요청마다 한 번 건드려 첫 POST 부터 헤더를 만들 수 있게 합니다.
화면 쪽은 `withCsrf()` 하나로 모았습니다 — `request()` 뿐 아니라 사진 업로드 둘과
로그아웃처럼 `fetch` 를 직접 부르던 세 곳도 같은 길을 지납니다.

#### 시험 도구가 시험을 가렸다

통합 시험 일곱 클래스가 토큰 없이 POST 를 던지고 있었습니다. 하나하나 붙이는 대신
`CsrfMockMvcCustomizer` 가 모든 요청에 <b>실제 쿠키·헤더 쌍</b>을 싣습니다.

처음에는 spring-security-test 의 `csrf()` 를 썼는데, 그것은 필터의 토큰 저장소를
<b>시험용으로 바꿔치기</b>합니다. 한 번 쓰고 나면 같은 컨텍스트에서는 실제 쿠키가
내려오는지 볼 수 없어, "셸이 쿠키를 내리는가" 시험이 <b>실행 순서에 따라</b> 통과하거나
깨졌습니다. 실제 저장소를 그대로 지나가는 쿠키·헤더 방식으로 바꾸고 의존성을 뺐습니다.

> **시험 도구가 운영 경로를 바꿔치기하면, 그 시험은 운영을 재는 것이 아니다.**

---

### I296. 운영 기본값이 열려 있었다 · **[확정 — 구현됨]** · [I95] 조정

세 가지가 "아무것도 안 정하면 위험한 쪽" 이었습니다.

| 것 | 전 | 후 |
|---|---|---|
| 회원가입 `MEMBERSHIP_SIGN_UP_OPEN` | `true` | **`false`** — 2인 전용 폐쇄형이다. 열 때만 명시한다 |
| live DB 자격 `DB_USERNAME`/`DB_PASSWORD` | `halley`/`halley` | **기본값 없음** — 없으면 기동이 실패한다 |
| 최초 admin 비밀번호 | 만들어서 INFO 로그 | `HALLEY_ADMIN_INITIAL_PASSWORD` 로 받으면 <b>로그에 남기지 않는다</b>. 없으면 예전처럼 만들어 적는다(WARN) |

환경변수를 빠뜨린 배포가 곧 가입이 열리고 기본 자격으로 DB 에 붙는 배포였습니다.
이제 빠뜨리면 <b>뜨지 않습니다</b> — 조용히 위험하게 뜨는 것보다 낫습니다.

비밀번호를 안 정하면 만들어서 로그에 적습니다(WARN) — 첫 부팅에서 읽을 다른 길이 없어서입니다.
"운영이라면 환경변수를 두라"고 같이 적습니다.

#### 조정 (2026-09-25) — admin 비밀번호는 강제하지 않는다

처음에는 live 에서 이 변수도 <b>없으면 기동 실패</b>하게 뒀는데, 첫 배포에서 그대로 걸렸습니다
(`Could not resolve placeholder 'HALLEY_ADMIN_INITIAL_PASSWORD'`). DB 자격과는 성격이 다릅니다 —
DB 자격이 기본값으로 뜨면 <b>매번</b> 위험하지만, admin 비밀번호는 <b>첫 부팅 한 번</b>
로그에 적히는 것이고 그 계정은 첫 로그인에서 바꾸게 돼 있습니다. 강제해서 얻는 것보다
배포가 막히는 비용이 컸습니다. 되돌립니다 — 주면 쓰고 로그에 안 남기되, 없어도 뜹니다.

`SignUpDefaultClosedTest` 는 <b>아무 속성도 덮어쓰지 않습니다</b> — 그래야 기본값을 재는
것입니다. 기존 `UserServiceTest` 는 스스로 가입하는 경로를 보는 시험이라 거기서 명시적으로
엽니다.
