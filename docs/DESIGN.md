...
**사전은 `data/sigungu-code.csv`로 함께 싣습니다.** `legal_dong_code`는 카카오로 채우는
지연 캐시라 기동 시 비어 있는데, 규제지역 적재는 기동 직후에 돕니다
(`SigunguCodeBootstrap` `@Order(10)` → `RegulatedAreaBootstrap` `@Order(20)`).

서울 전 자치구와 경기 시군구를 담았고 현행 고시 40곳이 모두 들어 있습니다. **다른 시도가
지정되면 매칭이 실패하고 그 이름이 로그에 남습니다** — 조용히 넘어가지 않으므로 그때 줄을
더하면 됩니다. 전국 사전이 필요해지면 행정표준코드관리시스템 API(data.go.kr 1741000)로
대체할 수 있습니다.
