...
#### 근거가 약해진 것 — [I239] 기준 정보 캐시

**정직하게 적습니다.** [I239]의 근거는 "왕복 20ms 라 요청마다 나가는 한 줌도
공짜가 아니다"였습니다. 그 20ms 가 사라졌습니다. 14행짜리 표를 Postgres 에 묻는 것과
Redis 에 묻고 JSON 을 푸는 것이 <b>이제 비슷합니다.</b>

**그런데 저는 로컬 H2 로만 잴 수 있습니다.** 재지 않고 걷어내는 것은 재지 않고
넣는 것과 같은 잘못입니다 — 이 문서의 첫 판이 "매물당 46개"라고 틀린 숫자를 적은 것이
정확히 그 잘못이었습니다.

그래서 **끄고 켤 수 있게** 했습니다.

```
HALLEY_CACHE_REFERENCE_ENABLED=false   # 같은 빌드로 껐다 켜며 잰다
```

- 꺼도 **답은 같습니다.** 늘 원본에서 읽을 뿐입니다
- 꺼도 **지우기는 돕니다.** 안 그러면 껐다 켠 순간 <b>끄기 전의 값</b>이 살아납니다
- 중앙값 차이가 **10ms 미만이면 걷어냅니다** — 낡을 위험만 남습니다
