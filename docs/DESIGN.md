...
모바일에서 사진을 업로드한 뒤 사진 모달을 닫으면 매물 상세가 PC처럼 두 열로 보이는 경우가
있었습니다. 상세 격자의 모바일 기준을 목록 시트·헤더와 같은 `767px`로 통일해, 사진 모달을
열고 닫거나 파일 선택 화면을 다녀와도 작은 화면에서는 상세 항목을 한 열로 유지합니다.
