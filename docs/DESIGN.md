...
### I287. 남이 먼저 매기면 <b>내가 못 매겼다</b> · **[확정 — 실물로 드러난 버그]** · [I111]·[I118] 후속

같은 그룹의 A 가 공간의 쾌적함을 매기면 B 는 매길 수 없었습니다.

#### 서버는 멀쩡했습니다

두 사람을 한 그룹에 넣고 A·B 가 차례로 매기는 통합 시험을 먼저 썼는데 <b>그대로
통과했습니다.</b> `user_criterion_score` 의 기본키는 `(매물, 사용자, 항목)` 이고
`myScore` 도 사용자별로 갈라 줍니다. 화면 문제였습니다.

#### 슬라이더가 가리키는 값과 저장될 값이 달랐다

내 점수가 없으면 `scoreForm` 은 빈 값입니다 — 남이 매긴 값을 내 것처럼 채우지 않으려는
것이라 <b>맞습니다</b>([I118]). 그런데 `<input type=range min=1 max=5 value="">` 는 값이
잘못됐다고 보고 브라우저가 <b>손잡이를 가운데(3)에 놓습니다.</b>

그래서 화면은 3 을 가리키는데 모델은 비어 있습니다. B 가 "3 이 맞다" 고 손대지 않고
저장하면, 저장 경로는 <b>바뀐 게 없다</b>고 읽고([I111]) 아무것도 보내지 않습니다.
누른 사람에게는 <b>저장이 안 되는 화면</b>입니다.

두 가지를 맞췄습니다.

- 슬라이더와 숫자가 <b>같은 값</b>을 보게 했습니다(`scoreSliderValue`). 예전에는 손잡이는
  가운데 있는데 숫자는 `–` 라, 저장해도 아무 일이 없는 이유를 화면에서 알 수 없었습니다.
- <b>아직 내 것이 비어 있는 쾌적함</b>은 손대지 않았어도 보냅니다.
- 시작값은 <b>가장 낮은 1</b>입니다. 가운데(보통)에서 시작하면 손대지 않고 저장했을 때
  "보통이라고 매겼다"가 되는데, 그건 사람이 고른 것이 아닙니다 — 올리는 것은 뜻이
  있어야 합니다.

#### 손잡이 자리를 점수로 보여 주면 안 된다

리뷰에서 잡혔습니다. 처음에는 슬라이더 값을 <b>그대로 숫자로도</b> 보여 줬는데, 그러면
쾌적함이 아닌 <b>미산출 항목이 0점으로 읽힙니다.</b> 슬라이더는 어떤 값이든 손잡이를
놓아야 하지만, 미산출은 <b>모르는 것이지 0점이 아닙니다</b>([I220] — 카드에서 이미 같은
혼동을 막았습니다).

<b>넣는 값과 보여 줄 값을 갈랐습니다.</b> `scoreSliderValue` 는 손잡이 자리를,
`scoreDisplayValue` 는 사람에게 보여 줄 것을 냅니다 — 쾌적함은 손대지 않아도 그 값으로
저장되므로 그대로 보여 주고, 미산출은 `–` 입니다. 읽어 주는 말도 "0점" 이 아니라
"아직 산출되지 않음" 입니다.

#### 시험이 도우미끼리만 견주고 있었다

같이 잡힌 것입니다. 첫 시험은 <b>표시값과 저장값을 같은 함수로 얻어</b> 견줬습니다 —
템플릿의 `:value`·`@input` 이 다시 어긋나도 통과합니다. 이번 버그가 바로 "DOM 슬라이더
값과 모델 값이 어긋난 것" 이었는데 말입니다.

하네스에 <b>템플릿 조각을 그대로 떼어 Alpine 으로 그리는</b> 길을 더했습니다
(`templateFragment` · `renderFragment`). 이제 진짜 `input` 요소의 값을 읽고, 진짜
`input` 이벤트를 흘려 모델이 받는지 봅니다 — `:value` 를 되돌리거나 `@input` 을 끊으면
시험이 잡습니다.

#### 미산출까지 그렇게 하면 안 된다

처음에는 "비어 있는 항목은 다 보낸다"로 고쳤는데, 시험을 하나 더 써 보고 <b>되돌렸습니다.</b>
미산출은 대개 그때 외부 조회가 실패한 것이라([I119]), 슬라이더 가운데 값이 그대로 저장되면
<b>실패가 사람이 매긴 점수로 굳습니다</b> — [I111]이 막으려던 바로 그 일입니다.
사람마다 따로 매기는 쾌적함에만 적용합니다.

> **화면이 가리키는 값과 저장될 값이 다르면, 사람은 자기가 뭘 잘못했는지 알 수 없습니다.**

---

### I288. 모바일에서 사진이 매물 정보를 밀어냈다 · **[확정 — 실물로 드러난 버그]** · [I203] 후속

모바일에서 상세를 열면 <b>사진이 있는 매물만</b> 정보가 안 보였습니다.

사진 띠(`.m2-photos`)는 가로로 이어 붙는 격자라 폭이 사진 수만큼 늘어납니다. 넓은 화면의
격자는 `repeat(2, minmax(0, 1fr))` 이라 칸이 <b>내용보다 좁아질 수 있어</b> 멀쩡했는데,
모바일 재정의가 `1fr` 이었습니다. `1fr` 은 `minmax(auto, 1fr)` 이라 좁아지지 않습니다.

브라우저로 재 봤습니다.

```
360px 화면 · 사진 6장
  격자 폭        950px   ← 카드(398px)를 한참 넘는다
  정보 칸 폭     950px   ← 옆으로 밀려 화면 밖
```

모바일 재정의를 `minmax(0, 1fr)` 로 고치고, 칸에도 `min-width: 0` 을 뒀습니다.
고친 뒤 격자는 362px 입니다.

> **둘 중 하나만 있어도 막힙니다.** 재서 확인했습니다 — 둘 다 두는 것은 격자 설정을
> 나중에 바꿔도 버티게 하려는 것이고, 주석에도 그렇게 적었습니다.

배치는 Java 로 재현할 수 없어 <b>막아 둔 표시가 남아 있는지</b>만 시험으로 지킵니다.
