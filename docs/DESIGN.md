...
V-World 토지이용계획(`getLandUseAttr`)을 실측했습니다(은마아파트 PNU, 35건).
투기과열지구·조정대상지역은 **한 건도 나오지 않았습니다.** 구조적인 이유가 있습니다 —
토지이용계획확인서는 「토지이용규제 기본법」 별표의 지역·지구, 즉 **토지 이용 행위를 제한하는**
것만 담습니다. 투기과열지구는 「주택법」상 지정이고 제한 대상이 금융·청약·전매라 별표에
들어가지 않습니다. 같은 "규제"라도 계통이 다릅니다.

data.go.kr의 `국토교통부_토지이용규제정보서비스`(15058410)도 **같은 LURIS 원천**이며,
그쪽은 "이 용도지역에서 어떤 행위가 가능한가"를 답하는 **행위제한 조회**라 성격이 더 멉니다.
