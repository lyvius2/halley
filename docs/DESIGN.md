...
**축약 지역명은 LLM이 법정동코드로 바꿉니다.** `화성동탄`을 어디서 자를지 문자열만으로는
알 수 없고(`화성/동탄`인지 `화성동/탄`인지), 시군구 사전을 코드에 박으면 동탄구처럼 새로 생기는
행정구역을 따라가지 못합니다. 무거운 판단이 아니라 `llm.claude.model.regulation`으로
가벼운 모델을 씁니다.
