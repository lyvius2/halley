...
> **작성일**: 2026-08-24 · **최종 갱신**: 2026-09-01 (I1~I237)
...
| **16장 (I1~I237)** | **결정 이력** | <b>여기가 현행입니다.</b> 뒤 번호가 앞 번호를 이긴다 |
...
| `<768px` | iPhone | 지도 위 **Bottom Sheet** (peek 25% → 헤더 아래 전체 확장) |

- CSS Grid + `clamp()` 기반, 프레임워크 없이 구현 가능
- 지도 컨테이너 리사이즈 시 `map.relayout()` 호출 필수
- 터치: 손잡이를 위·아래로 밀어 Bottom Sheet 확장·축소, 마커 탭 → 자동 확장
- 모바일 헤더는 한 줄 고정 높이로 유지하고 보조 메뉴는 더보기 안에 둠
...
### I234. 판단 보류가 너무 많았다 · **[확정 — 구현됨]** · [I130]·[I151] 조정

**거의 모든 매물이 `판단 보류`였습니다.** 금리·전세가율·용도지역 같은 지표가
<b>여럿 나와 있는데도</b> 그랬습니다 — 실거래 표본이 얇다는 이유 하나로 덮었습니다.

[I130]에서 "UNCERTAIN 은 1급 시민"이라고 정한 것은 <b>지어낸 방향을 막으려는</b>
것이었습니다. 그런데 실제로는 <b>알아낸 것까지 감추고</b> 있었습니다.
...
동수일 때
  ① 상승이 끼어 있으면 → 상승
  ② 상승이 없고 유지·하락이 갈리면 → 유지
```

①의 이유: <b>오를 수도 있다는 신호를 묻어 두면 기다리다 놓칩니다.</b>
이 도구는 살 집을 고르는 자리입니다.
②의 이유: <b>하락을 단정하려면 그쪽이 더 많아야</b> 합니다.

**무게를 쓰지 않고 그냥 셉니다.** 무게를 곱하면 "실거래 추세 하나가 나머지 셋을
이긴다" 같은 일이 생기는데 그 비율을 정당화할 근거가 없습니다.
(규칙 예측이 무게를 쓰는 것은 <b>다른 목적</b>입니다 — 거기서는 표차로 확신도를 잽니다.)

#### 그래도 감추지 않는 것

- **확신도는 LOW 입니다.** 세어서 고른 것이지 확신이 있어서가 아닙니다
- **왜 그렇게 정했는지 적습니다** — "실거래 표본이 3건 미만이라 지표 4개가
  가리키는 쪽을 세어 정했습니다 — 확신이 있어서가 아닙니다"
- **셀 것이 하나도 없으면 여전히 `UNCERTAIN`** 입니다. 다수결로 바꿨다고
  <b>없는 방향을 지어내지는 않습니다</b>

#### `횡보` → `유지`

시세 용어라 한 번 더 생각하게 만듭니다. 화면에 뜨는 말은 <b>바로 읽혀야</b> 합니다.
카드의 화살표(`▶`)와 색은 그대로입니다.

---

### I235. 네이버 검색이 API Hub 로 옮겨졌다 · **[확정 — 구현됨]**

```
WARN NaverSearchFallbackFactory  Naver news search failed …
     [401 Unauthorized] {"errorMessage":"NID AUTH Result Invalid (1000) : 인증에 실패했습니다.","errorCode":"024"}
```

키를 새로 발급받아도 같았습니다. **주소가 폐기된 것**이었습니다 —
`openapi.naver.com` 에서 네이버 클라우드 <b>API Hub</b> 로 옮겨졌고,
<b>헤더 이름도 함께 바뀌었습니다.</b>

같은 키로 두 주소를 나란히 불러 확인했습니다.

```
openapi.naver.com/v1/search/news.json      X-Naver-Client-Id/Secret       → 401
naverapihub.apigw.ntruss.com/search/v1/news  X-NCP-APIGW-API-KEY-ID/KEY   → 200
```

오류 문구가 <b>"인증에 실패했습니다"</b> 뿐이라 키를 의심하게 만듭니다.
주소가 죽었다는 말은 어디에도 없습니다.

- **바뀐 것은 주소와 헤더뿐**입니다. 응답 구조는 그대로라 파서는 손대지 않았습니다
- 경로가 `.json` 으로 끝나지 않습니다. `format` 은 기본이 `json` 이라 <b>안 보냅니다</b> —
  기본값을 굳이 적으면 기본이 바뀌었을 때 못 알아챕니다
- **환경변수 이름은 그대로 뒀습니다.** 담는 것이 같고, 이름을 바꾸면
  <b>배포된 설정이 조용히 비어</b> 기사만 사라집니다

#### 파서 테스트는 멀쩡한 채로 통과하고 있었다

깨진 곳은 <b>주소와 헤더</b>인데, 있던 테스트는 <b>응답 파싱</b>만 봤습니다.
`NaverApiHubContractTest` 를 따로 뒀습니다 — 애너테이션을 읽어 <b>어디로 무슨
헤더로 부르는지</b>만 봅니다. 외부가 옮겨 가는 일은 또 있습니다.

---

### I236. 애플펜슬로 코멘트 쓰기 · **[확정 — 구현됨]**

**따로 붙일 것이 없었습니다.** 코멘트 칸이 표준 `<textarea>` 라 iPadOS 의
**Scribble** 이 그대로 동작합니다 — 손글씨를 글자로 바꿔 넣어 줍니다.
막을 만한 것(`pointerdown` 가로채기·`user-select:none`)도 그 자리엔 없습니다.

**실제 위험은 다른 데 있었습니다.**

| | 왜 |
|---|---|
| 획이 칸 밖으로 삐져나가면 | 배경 클릭으로 읽혀 <b>모달이 닫히고 글이 사라집니다</b> ([I122]에서 배경 닫기를 되살렸습니다) |
| 네 줄짜리 칸 | 손글씨는 타자보다 큽니다 — 한 줄 쓰고 스크롤됩니다 |

- **쓰다 만 글을 브라우저에 담아 둡니다.** 닫혀도, 새로고침해도 다시 열면 그대로입니다.
  **서버에 두지 않습니다** — 아직 남기지 않은 글은 남의 눈에 보이면 안 됩니다
- **손가락·펜 기기에서만 칸을 키웁니다**(`@media (pointer: coarse)`).
  마우스 쓰는 화면까지 넓힐 이유는 없습니다

> 사파리 비공개 모드에서는 `localStorage` 가 막힙니다. **담아 두지 못할 뿐**
> 쓰는 데는 지장이 없게 했습니다.

---

### I237. 놓인 자리에 따라 칸 모양이 달라졌다 · **[확정 — 구현됨]**

회원 탈퇴의 비밀번호 칸만 <b>브라우저 기본 모양</b>으로 떴습니다.

```css
:is(.form, .form-grid) :is(input…) { … }   ← 이 안에 있을 때만 걸린다
```

같은 `.group-name-row` 를 쓰는 다른 셋(그룹 이름·Slack 웹훅·초대 코드)은
모두 `.form-grid` 안에 있었고, **탈퇴 칸만 밖에** 있었습니다.

`.group-name-row` 가 <b>스스로</b> 칸 모양을 정하게 했습니다.
**놓인 자리에 따라 생김새가 달라지면 옮길 때마다 다시 깨집니다.**

---

### I238. 캐시를 얹기 전에, 내가 만든 N+1을 먼저 걷어냈다 · **[확정 — 구현됨]** · [I124] 재발

`ADJUST_CACHE.md` §2.1(기준 정보 캐시)을 하려다 <b>먼저 쟀습니다</b>. 그 문서의 원칙이
그것이고, 첫 판의 "매물당 46개"가 **틀린 숫자**였던 것도 재지 않고 세었기 때문입니다.

```
매물 6건 → criterion_weight 조회 7회
```

**1 + 매물 수.** 정확히 N+1입니다. [I124]에서 걷어낸 것을 <b>[I199]에서 정렬을
넣으며 제가 다시 만들었습니다</b> — `buildFromPersisted()` 가 매물마다
`byPriorityRank()` 를 부르고, 그것이 `criterion_weight` 전체를 읽습니다.

14행짜리 표라 눈에 안 띕니다. 그래서 <b>두 번</b> 만들었습니다.

목록 한 번에 순위표를 한 번만 읽고, 그것을 끝까지 나릅니다.

#### 캐시가 아니라 구조를 먼저 고친 이유

캐시를 먼저 얹었다면 **N+1은 그대로 남은 채 숫자만 좋아졌을 것**입니다. 그리고
캐시가 비는 순간(배포 직후·수명 만료·Redis 장애) 그대로 돌아옵니다.
당시 운영의 Redis 는 **실제로 죽어 있었습니다**(`ADJUST_CACHE.md` §3).

> **[I242]** Redis 는 살아났지만 이 결론은 바뀌지 않습니다. **왕복이 빨라져도
> `1 + 매물 수`는 잘못된 구조**이고, 빨라진 것은 그 잘못이 <b>안 보이게</b> 만들 뿐입니다.

#### 재는 테스트에서도 캐시를 꺼야 했다

캐시([I239])를 얹고 나니 **N+1을 되돌려도 이 테스트가 통과**했습니다 — 담아 둔 것이
쿼리를 가려서, 구조가 멀쩡한지를 못 보게 됐습니다. 테스트에 <b>아무것도 담지 않는
캐시</b>를 물려 <b>캐시가 비었을 때의 부하</b>를 재도록 고쳤습니다.

> 돌연변이로 확인했습니다. 캐시를 켠 채로는 이 돌연변이가 <b>살아남았습니다.</b>

---

### I239. 기준 정보를 담아 두되, <b>저장소 안에서</b> 담는다 · **[확정 — 구현됨]** · `ADJUST_CACHE.md` §2.1

사람이 손대야만 바뀌는 다섯 표를 [I180]의 `CachePort` 위에 얹었습니다.

| 표 | 담는 것 | 수명 | 버리는 때 |
|---|---|---|---|
| `criterion` | 목록 전체 | 1시간 | 항목 저장·삭제 |
| `criterion_weight` | 목록 전체 | 1시간 | 가중치 저장·삭제·전체삭제 |
| `regulation_param` | 프로파일별 | 1시간 | 저장·수정·삭제 |
| `system_config` | 키별 + 전체 | 10분 | 저장·수정·삭제 |
| `legal_dong_code` | 코드별·동별 <b>낱개</b> | 24시간 | 그 코드만 |

#### 부르는 쪽이 아니라 저장소에 넣었다

`criterion` 은 9곳, `system_config` 은 12곳에서 읽습니다. **부르는 쪽마다 감싸면
어느 하나가 빠집니다.** 더 나쁜 것은 <b>지우는 쪽</b>입니다 — 저장하는 자리 중
한 군데만 빠뜨려도 그때부터 옛 값으로 계산합니다.

읽는 곳과 지우는 곳을 <b>한 클래스 안에</b> 두면 둘이 어긋날 자리가 없습니다.
[I230]·[I232]·[I237] 이 모두 <b>같은 규칙이 두 벌</b>이어서 난 사고였습니다.

#### 커밋 뒤에 한 번 더 지운다

쓰기 시점에만 지우면 **커밋 전에 끼어든 읽기가 옛 값을 다시 담습니다.** 그러면
수명이 다할 때까지 틀린 값입니다. 트랜잭션 안이면 완료 후 한 번 더 지웁니다 —
지우는 일은 싸니 두 번 합니다.

#### "없다"도 담는다

`system_config` 과 `legal_dong_code` 는 <b>못 찾는 경우가 잦습니다.</b> 없을 때만
원본까지 가면 캐시가 아무 일도 안 합니다. [I219]에서 실거래에 쓴 처방과 같습니다.
대신 **새로 넣을 때 반드시 버려야** 합니다 — 안 그러면 10분간 "없다"고 답합니다.

#### 사전만 낱개로 담는다

`legal_dong_code` 는 칸이 수만 개입니다. 여기서 갈래를 통째로 버리면(Redis `SCAN`)
**한 건 저장할 때마다 전부 훑습니다.** 저장한 코드와 동 이름은 알고 있으니
<b>그 두 칸만</b> 버립니다. 목록 전체(`findAll`)는 담지 않습니다 — 한 덩어리로
담을 크기가 아니고, 부르는 곳도 관리자 작업 한 군데뿐입니다.

#### 실패해도 값은 온다

캐시를 못 읽거나 못 쓰는 것은 <b>느려질 뿐</b>입니다. 당시 운영의 Redis 가 죽어 있는
상태에서도 채점과 대출 계산은 돌았습니다 — Redis 컨테이너를 내려도 마찬가지여야 합니다.

#### 쓰는 길마다 따로 확인했다

지우는 곳이 여럿인데 한 테스트로 묶으면 <b>하나만 지워도 통과</b>합니다.
실제로 처음에 그렇게 짜서, "저장할 때 안 지운다"는 돌연변이가 <b>살아남았습니다</b>
(앞선 `delete` 가 지워 버려서). 저장·삭제를 갈라 각각 봅니다.

---

### I240. 매물 목록을 30건씩 자른다 · **[확정 — 구현됨]** · [I221] 조정

목록이 <b>페이징 없이</b> 전부 내려오고 있었습니다. 매물 하나에 채점 14줄과 전망
요약이 붙으니, 건수가 늘면 첫 화면이 그만큼 늦어집니다.

```
GET /api/properties?sort=PRICE&page=0&size=30
→ { items: [...30건], page, size, total, hasNext }
```

#### 줄 세우기를 서버로 옮겼다

[I221]에서는 <b>화면이</b> 줄 세웠습니다. 목록이 통째로 왔으니 그때는 맞았습니다.
30건씩 자르는 순간 <b>틀린 설계</b>가 됩니다 — 받은 30건 안에서만 세우면
**2쪽의 1등이 1쪽의 꼴찌보다 앞에 옵니다.**

> **자르는 곳과 줄 세우는 곳은 같아야 합니다.**

기본 정렬이 "안 가 본 곳 먼저"라 <b>임장 여부가 정렬의 입력</b>입니다. 그 판단이
[I226]에서 <b>화면에만</b> 있었으므로 서버로 옮겼습니다 — 방문 기록이 있거나
쾌적함을 매겼으면 가 본 것입니다.

#### SQL 로는 자를 수 없다

총점은 어디에도 저장하지 않고 <b>읽을 때마다</b> 계산합니다([I173]). 그래서
`LIMIT/OFFSET` 으로는 못 자릅니다 — 전체를 채점해 줄 세운 뒤 자릅니다.

**그래서 이 자르기가 줄이는 것은 왕복 크기와 화면이 그리는 양입니다.** 쿼리 수는
이미 매물 수와 무관합니다([I124]·[I238]). 전망 요약도 <b>이 쪽의 30건에만</b>
붙습니다 — 예전에는 전체에 붙였습니다.

#### 잘라도 지도는 전부 본다

지도·임장 플래너·비교 우위 대상 수는 <b>전체</b>를 알아야 합니다. 잘린 목록으로
그리면 **매물이 사라진 것처럼** 보입니다. 좌표만 담은 얇은 목록을 따로 냅니다.

```
GET /api/properties/pins → id·이름·좌표·가격·면적·다녀왔는지
```

채점까지 붙은 전체 목록을 또 받으면 자른 보람이 없습니다.

#### 겹쳐 와도 두 번 안 나오게

뒤에서 채점이 끝나 순서가 조금 밀리면([I85]) 같은 매물이 두 쪽에 걸립니다.
그냥 이어 붙이면 <b>화면에 두 번</b> 나옵니다 — 이미 있는 id 는 버립니다.
반대로 <b>어느 쪽에도 안 나오는</b> 쪽이 더 무섭습니다. 그건 아무도 모릅니다.
그래서 테스트가 "쪽을 다 이으면 하나도 빠지지 않는다"를 <b>다섯 정렬 모두</b>에서 봅니다.

#### 감시점은 늘 자리에 둔다

바닥이 보이면 다음 쪽을 부릅니다({@code IntersectionObserver}). 스크롤 위치를
직접 재지 않는 이유는, 이 화면이 <b>넓을 때는 목록 칸이, 좁을 때는 창 자체가</b>
스크롤되기 때문입니다 — 두 경우를 손으로 가르면 한쪽이 반드시 어긋납니다.

감시점을 `display:none` 으로 감추면 <b>관찰이 풀려 다음 쪽을 영영 안 부릅니다.</b>
늘 자리에 두고, 안내 문구만 바꿉니다.

#### 몇 건인지 다시 말한다

머리말이 `visibleProperties.length + '건'` 이었습니다. 그대로 두면 35건인데
<b>"30건"이라고 말합니다.</b> `30 / 35건` 으로 바꿨습니다 — 서버가 세어 준 값입니다.
화면이 다시 세면 <b>세는 규칙이 두 벌</b>이 됩니다.

#### 화면이 시키는 대로 다 주지 않는다

`size` 는 100 에서 자릅니다. `size=100000` 한 번에 전부 받아 가면 [I219]에서
국토부에 그랬듯 <b>자른 척만</b> 하게 됩니다.

#### 테스트 두 개가 안 재고 있었다

돌연변이로 확인하다 두 개가 <b>아무것도 가르지 않고</b> 있었습니다.

- "겹쳐 와도 두 번 안 나온다" — 겹치는 응답을 만들지 못해 <b>겹친 적이 없었습니다</b>
- "안 받은 쪽의 매물도 다녀온 것으로 본다" — 하필 <b>첫 쪽에 있는</b> 매물로 봤습니다

둘 다 통과하고 있었습니다. **통과한다는 것과 재고 있다는 것은 다릅니다.**

---

### I241. 안 볼 매물을 <b>치워 둔다</b> · **[확정 — 구현됨]**

목록에 안 볼 매물이 계속 남아 있었습니다. **삭제는 되돌릴 수 없어** "일단 안 보이게"에
쓸 수 없습니다 — 코멘트도 채점도 실거래 이력도 함께 사라집니다.

```
매물 카드:  상세 · 완성 · 코멘트 · 채점 · 대출 · 실거래 · 수정 · 아카이브 ⟨          ⟩ 삭제
탭:        전체 · 매매 · 전세 · 아카이빙 ③
```

- **아카이브는 수정 옆**에 둡니다. 되돌릴 수 있는 일입니다
- **삭제만 맨 오른쪽**으로 떼어 둡니다. 되돌릴 수 없는 것은 손이 미끄러지는 자리에
  두지 않습니다
- 치우기는 묻고, **되돌리기는 안 묻습니다.** 되돌리는 일은 되돌릴 수 있습니다

`ListingStatus.ARCHIVED` 와 `PATCH /{id}/status` 는 <b>이미 있었습니다.</b>
없던 것은 <b>걸러내기와 화면</b>뿐이었습니다.

#### 거르는 곳은 한 군데

```java
byGroup(dealType).stream()
    .filter(p -> (p.listingStatus() == ARCHIVED) == archived)
```

저장소의 조회 메서드(`findAll`·`findByDealType`·`findByGroupId`·`findByGroupIdAndDealType`)
마다 조건을 붙이면 **넷 중 하나를 반드시 빠뜨립니다.** 그러면 다른 탭에서 되살아납니다 —
테스트가 거래유형 탭까지 보는 이유입니다.

#### 뱃지는 어느 탭에서 봐도 실려 온다

`archivedTotal` 을 목록 응답에 <b>늘</b> 담습니다. 치운 것이 몇 건인지 모르면
**치웠다는 사실 자체를 잊습니다.** 0건이면 뱃지를 안 답니다.

노란색입니다 — 지운 것이 아니라 <b>치워 둔 것</b>이라는 표시입니다.

#### 지도도 같이 사라진다

목록에서 사라졌는데 지도에 남아 있으면, 눌러도 <b>카드가 없습니다.</b>
얇은 목록(`pins`)도 같은 조건으로 받습니다.

#### 판 번호도 같은 것을 세게 했다

`scoreVersions()` 가 `findAllIds()` 로 <b>모든 그룹의 모든 매물</b>을 세고 있었습니다.
화면은 그 개수를 자기 목록 길이와 견주어 "매물이 늘거나 줄었다"를 판단하므로,
세는 대상이 다르면 **3초마다 목록을 통째로 다시 받습니다.**

전에도 <b>거래유형 탭에서는 이미 그러고 있었습니다.</b> 목록을 30건씩 자르기
시작하니([I240]) 전체 탭에서도 드러났습니다. 목록과 <b>같은 조건</b>으로 묻게 했습니다.

> 이 프로젝트에서 반복되는 모양입니다 — **세는 것이 둘이면 반드시 어긋납니다**
> ([I230]·[I232]·[I237]·[I239]).

---

### I242. DB·Redis 가 같은 EC2 로 왔다 — <b>전제가 바뀌었다</b> · **[확정]** · [I125]·[I239] 조정

PostgreSQL 과 Redis 를 **앱과 같은 EC2 안 Docker** 로 올렸습니다.
성능 결정 여럿이 <b>느린 원격 DB</b>를 전제로 쓰여 있었습니다.

| 전제 | 그때 | 지금 |
|---|---|---|
| DB 왕복 | 20ms (무료 클라우드) | **0.1~1ms** |
| `max_connections` | 20~30 (무료 등급) | **100** (직접 조절) |
| Redis | **죽어 있었음** | 살아 있음 |

#### 그대로인 것

**[I238] N+1 제거와 [I240] 목록 자르기는 바뀌지 않습니다.**

왕복이 빨라져도 `1 + 매물 수`는 잘못된 구조입니다 — 빨라진 것은 그 잘못이
<b>안 보이게</b> 만들 뿐입니다. [I240]이 줄인 것도 DB 시간이 아니라
<b>왕복 크기와 브라우저가 그리는 양</b>이었습니다.

#### 뒤집힌 것 — 커넥션 풀 ([I125] 조정)

`maximum-pool-size` 를 **5 → 10**. 5로 묶은 근거였던
`remaining connection slots are reserved for SUPERUSER` 는 <b>무료 등급의 증상</b>이었고,
자체 호스팅에는 그 제약이 없습니다.

기준은 HikariCP 공식 `(vCPU × 2) + 디스크`. 다만 **Postgres 가 앱과 같은 코어를 나눠
씁니다** — 전용 DB 호스트가 아니므로 <b>공식값 위로 가지 않습니다.</b>
vCPU 2 기준 10입니다.

> **더 늘리는 것은 손해입니다.** 쿼리가 1ms 아래라 커넥션 하나가 초당 수천 번 돕니다.
> 늘리면 연결당 메모리(5~10MB)와 문맥 전환만 늘어납니다.

#### [I239] 기준 정보 캐시는 <b>그대로 씁니다</b>

[I239]의 근거로 "왕복 20ms"를 들었는데, 그 20ms 는 사라졌습니다.
**그렇다고 캐시를 걷어낼 이유가 되지는 않습니다.**

> 왕복이 싸졌다고 해서, <b>사람이 안 건드리는 표를 요청마다 다시 물을 이유가
> 생기는 것은 아닙니다.</b>

`criterion` 14행 · `criterion_weight` 14행은 관리자가 손대야만 바뀝니다. 무효화는
쓰기마다 · 커밋 뒤 한 번 더로 촘촘하고, 실패해도 값은 옵니다. 남길 이유는 충분합니다.

제가 처음에 "10ms 미만이면 걷어낸다"고 <b>결론을 미리 적어 둔 것</b>은 잘못이었습니다.
근거 하나가 약해진 것과 결정이 뒤집히는 것은 다릅니다.

#### 다만 껐다 켤 수는 있어야 합니다

나중에 "느린 게 캐시 탓인가"를 물을 때 <b>가를 수 있어야</b> 합니다.

```yaml
halley:
  cache:
    reference:
      enabled: ${HALLEY_CACHE_REFERENCE_ENABLED:true}
```

- **기본은 켜짐**입니다
- 꺼도 **답은 같습니다.** 늘 원본에서 읽을 뿐입니다
- 꺼도 **지우기는 돕니다.** 안 그러면 껐다 켠 순간 <b>끄기 전의 값</b>이 살아납니다

> **처음에 이것을 `application.yaml` 에 안 적었습니다.** `@Value` 에만 두고
> 환경변수 이름은 <b>문서에만</b> 썼습니다. Spring 이 이름을 바꿔 찾아 주기는 하지만,
> 이 프로젝트는 조절할 값을 전부 yaml 에 `${ENV:기본값}` 으로 적어 둡니다 —
> **그래야 grep 으로 잡히고** 무엇을 넣을 수 있는지 한 곳에서 보입니다.
> **문서에만 있는 스위치는 없는 스위치입니다.**

절차는 `ADJUST_CACHE.md` §5 에 있습니다.

#### 아직 안 잰 것 — 보정 400 : 커넥션 10
...
보정은 외부 API 대기가 대부분이라 커넥션을 오래 쥐지 않지만, 매물을 한꺼번에
등록하면 몰릴 수 있습니다. **`HikariPool` 로그의 `waiting` 만 보면 됩니다** —
늘 0이면 문제가 아니었던 것입니다.

> **풀부터 늘리지 않습니다.** 400개가 동시에 DB 를 두드리는 것이 정상인지부터
> 물어야 합니다. 낮출 것은 `ENRICHMENT_MAX_CONCURRENCY` 쪽이 먼저입니다.

---

### I243. 커넥션 풀이 마르는지 <b>앱이 스스로 본다</b> · **[확정 — 구현됨]**
...
`ADJUST_CACHE.md` §5.2 에 <b>코드를 안 고쳐도 되는</b> 방법을 적었습니다.

```yaml
com.zaxxer.hikari.pool.HikariPool: DEBUG   # 30초마다 풀 상태를 찍는다
```

**보정 폭주는 몇 초짜리입니다.** 두 샘플 사이에 일어나면 통째로 안 보이고,
하루 내내 `waiting=0` 만 보고 <b>"문제없다"고 잘못 결론</b>냅니다.

> 못 본 것을 <b>없었던 것</b>으로 읽습니다. 이 프로젝트가 반복해서 겪은 실패의
> 모양입니다 — [I219](10건만 받고 있었다), [I230](규칙이 두 벌이었다).

**1초로 봅니다.** `connection-timeout` 이 3초라 <b>1초를 못 넘기는 대기는 애초에
아무도 못 죽입니다</b> — 해가 될 수 있는 것은 전부 걸립니다.

#### 제 스레드에서 돈다

`@Scheduled` 로 달았다가 뺐습니다. 이 앱의 스케줄러는 <b>스레드 하나</b>이고
(`spring.task.scheduling.pool.size` 기본값) 거기 배치가 여섯 개 물려 있습니다.
전망 배치처럼 오래 도는 것이 잡고 있으면 **그동안 감시가 멈춥니다** —
하필 DB 가 바쁜 그때입니다. <b>30초 샘플링과 같은 잘못</b>이 됩니다.

가상 스레드 하나가 정수 셋을 읽고 잡니다. 값이 없습니다.

> 겸사겸사 스케줄러 풀을 <b>2</b>로 올렸습니다. 새벽 4시에 잡 셋이 몰려 있는데
> 하나로 돌면 먼저 잡은 것이 끝날 때까지 나머지가 밀립니다.

#### 조용하다

- 줄 선 것이 **둘 이하면 아무 말도 안 합니다.** 커넥션이 오가며 잠깐 겹치는 것은
  정상이고, 그것까지 찍으면 <b>진짜 문제가 소음에 묻힙니다</b>
- 마르는 동안 1초마다 찍으면 로그가 못 쓰게 되므로 **1분에 한 번**만 말합니다
- 최고치는 **기억해 둡니다.** 폭주는 새벽에도 일어나고 그때 사람이 보고 있을 리
  없습니다. 매일 5시에 <b>볼 것이 있을 때만</b> 남기고 0으로 되돌립니다

> "괜찮았다"를 매일 남길 이유가 없습니다. **남아 있는 줄이 곧 무슨 일이 있었다는
> 뜻**이어야 합니다 ([I162]와 같은 태도).

#### 감시가 앱을 죽이지 않는다

Hikari 가 아니거나 풀이 아직 안 떴으면 <b>조용히 안 봅니다.</b> 다만 그때는
<b>보고 있다고 착각해서도 안 되므로</b> 기동 때 한 번 말합니다.

---

### I244. 주소를 `#` 뒤로, 그리고 <b>넷만</b> · **[확정 — 구현됨]** · [I188]·[I198] 조정

`spring-ai-ops` 와 같은 방식으로 맞춥니다.

```
domain.com                              목록 (기본)
domain.com/#/list                       매물 리스트
domain.com/#/tour-plan                  임장 플래너
domain.com/#/properties/12              매물 상세
domain.com/#/properties/12/loan         대출
domain.com/#/properties/12/score        채점
domain.com/#/properties/12/comments     코멘트
```

#### 열일곱에서 셋으로

모달마다 주소를 준 것은([I198]) <b>"Slack 이 알린 그 자리로 바로"</b> 가 목적이었습니다.
그런데 실제로 알림이 가리키는 곳은 <b>코멘트와 채점뿐</b>입니다 —
나머지 열넷(실거래·전망·사진·평면도·중개사·로드뷰·수정·붙여넣기·비교·사용자·비밀번호·
가입·설정)은 <b>아무도 링크로 건넨 적이 없습니다.</b>

주소가 있으면 그만큼 지켜야 합니다. 모달 하나에 <b>열고·닫고·되돌아오고·딥링크로
들어오는</b> 네 경로가 생깁니다. **쓰지 않는 것을 지키는 것은 비용만 남습니다.**

화면 주소도 다섯에서 둘로 줄였습니다. 내 정보·그룹·가중치는 <b>남에게 건넬 화면이
아닙니다.</b>

#### `#` 뒤는 서버로 가지 않는다

전에는 `ViewController` 에 주소 열여덟 개가 나열돼 있었습니다.
**동작의 근거는 아니었습니다** — 딥링크를 받아 준 것은 `SpaRoutingFilter` 이고,
그 목록은 "어떤 주소가 있는지 적어 둔 것"이었습니다.

> 그런데 그것이 `app.js` 의 `ROUTES` 와 <b>같은 것을 두 곳에 적는</b> 상태였습니다.
> 늘리려면 둘 다 고쳐야 했고, <b>하나를 빠뜨려도 아무 일도 안 일어나</b> 틀린 줄도
> 몰랐습니다. 이 프로젝트가 반복해서 겪은 모양입니다([I230]·[I232]·[I239]).

이제 주소는 `app.js` 한 곳에만 있고, 서버 매핑은 `/` 하나입니다.

#### 미는 방식은 안 바꿨다

`spring-ai-ops` 는 여는 자리마다 `updateHash(...)` 를 부릅니다(14군데).
**우리는 상태에서 계산하는 방식을 지킵니다**([I198]).

셋으로 줄었어도 여는 자리와 닫는 자리를 세면 <b>여덟 곳</b>이고, 그중 하나만
빠뜨려도 <b>주소가 조용히 어긋납니다</b> — 그리고 어긋난 주소는 아무 증상이 없습니다.
주소의 모양을 맞추는 것과 <b>주소를 미는 방식</b>은 별개입니다.

#### 닫으면 벗겨진다

채점을 닫으면 `#/properties/12`, 상세까지 닫으면 `#/list` 입니다.
**주소는 늘 지금 보이는 것을 가리킵니다.**

`#` 가 비어 있으면 목록입니다 — 처음 들어온 사람에게 `#/list` 를 <b>억지로 붙이지
않습니다.</b> 아무것도 안 한 사람의 주소는 깨끗한 편이 낫습니다.

#### 알림 링크도 같이 옮겼다

```java
sb.append(baseUrl).append("/#/properties/").append(propertyId).append(suffix);
```

**이걸 안 바꾸면 링크를 눌러도 껍데기만 뜹니다.** 서버는 `#` 뒤를 못 보고, 앱은
`#` 가 비었으니 목록을 엽니다 — 누른 사람은 <b>아무 일도 안 일어난 것</b>으로 봅니다.

> 이미 나간 옛 링크(`/properties/12/score`)는 **200 은 받지만 목록이 열립니다.**
> 사용자 판단으로 리다이렉트는 넣지 않았습니다 — 수가 적고 아무도 안 봅니다.

---

### I245. 다녀온 곳이 안 가 본 곳을 가리고 있었다 · **[확정 — 구현됨]**

지도에서 핀이 겹쳤을 때 <b>흐린 것(다녀온 곳)이 진한 것(안 가 본 곳)을 덮었습니다.</b>

```
  ┌─────────────┐
  │ 60㎡ 매 9.1억 │  ← 다녀온 곳 (흐림)
  ├──┬──────────┴──┐
  │  │ 59㎡ 매 7.6억 │  ← 안 가 본 곳이 가려졌다
  └──┴─────────────┘
```

**가려도 되는 것이 가려야 합니다.** 임장은 <b>아직 안 가 본 곳을 고르려고</b> 보는
지도입니다. 이미 다녀온 곳이 그것을 덮으면 <b>정작 볼 것이 안 보입니다</b> —
흐리게 칠한 것([I225])과 같은 이유인데 <b>절반만 하고 있었습니다.</b>

#### 앞뒤가 운에 달려 있었다

`CustomOverlay` 에 `zIndex` 를 안 줬습니다. 그러면 <b>만든 차례대로</b> 쌓이고,
그 차례는 그저 `pins` 가 온 순서입니다 — **무엇이 위에 올지가 정해져 있지 않았습니다.**

#### 순서를 바꾸는 대신 순서에서 떼어 냈다

"안 가 본 것을 <b>나중에</b> 만들면 되지 않나" — 됩니다. 그런데 두 가지가 걸립니다.

- **`pins` 는 임장 플래너 체크박스 목록도 씁니다**([I240]). 지도 쌓임을 위해 정렬하면
  <b>플래너 목록 순서까지 바뀝니다</b>
- 순서에 기대면 규칙이 <b>멀리 떨어진 곳에 숨습니다.</b> 나중에 누가 다른 이유로
  `pins` 를 정렬하면 지도가 조용히 되돌아가고, **증상이 없어 아무도 모릅니다**

오버레이마다 `zIndex` 를 못 박았습니다. 테스트가 <b>배열을 뒤집어도 앞뒤가 같은지</b>를
봅니다.
...
#### `zIndex` 를 주면 hover 가 죽는다

`.map-pin:hover { z-index: 10 }` 이 <b>먹고 있던 이유</b>는 오버레이 컨테이너에
`z-index` 가 없어서 자식이 상위 스택에 직접 참여했기 때문입니다.
오버레이에 `zIndex` 를 주는 순간 **컨테이너가 쌓임 맥락을 만들고**, 그 안의
`z-index: 10` 은 <b>형제 오버레이와 겨루지 못합니다.</b>

고치면서 <b>다른 것을 조용히 깨뜨릴 뻔했습니다.</b> `mouseenter`/`mouseleave` 로
오버레이째 올리고 내립니다. CSS 쪽에는 왜 여기서 못 정하는지를 적어 두었습니다.

---

> **I244·I245 는 다른 갈래에 있습니다** — `feature/2026.09.04_Routing`(I244),
> `bugfix/2026.09.03_picker`(I245). 이 갈래는 그보다 앞에서 나왔습니다.

### I246. 관련 기사를 하루 담아 둔다 · **[확정 — 구현됨]** · [I137] 보완

매물 상세를 <b>열 때마다</b> 네이버에 물었습니다. 같은 매물을 몇 번 열든
<b>기사는 그대로</b>이고, 네이버 검색에는 일일 호출 한도가 있습니다.

```
캐시 확인 → 없으면 네이버 → 담기        (Cache-Aside)
TTL 24시간 · 키는 검색어
```

#### 키는 매물 번호가 아니라 <b>검색어</b>

검색어는 `강남구 대치동 + 단지명` 입니다([I137]). **같은 단지의 매물 둘은 검색어가
같습니다** — 매물 번호로 담으면 <b>같은 답을 두 번</b> 받아 옵니다.

`limit` 을 키에 넣습니다. 설정을 바꿨는데 <b>옛 개수로 담아 둔 것</b>이 나오면
안 됩니다.

#### 실패는 담지 않는다 — 여기가 이 캐시의 급소

`search()` 는 <b>네 가지 경우에 모두 빈 목록</b>을 돌려줍니다.

| 무엇 | 담나 |
|---|---|
| 키가 없다 | 애초에 안 부릅니다 |
| 폴백이 돌았다(`body == null`) | **아니오** |
| 응답이 오류다(`errorMessage`) | **아니오** |
| 본문이 깨졌다 | **아니오** |
| **진짜로 기사가 0건이다** | **예** |

그것까지 담으면 <b>네이버가 잠깐 죽은 것 때문에 하루 종일 "기사 없음"</b>이 됩니다.

#### 0건은 왜 담는가

> "실패했거나 결과가 0건이면 안 담으면 되지 않나요?"

**0건은 담습니다.** 기사가 없는 단지는 <b>앞으로도 없습니다.</b> 안 담으면 그 매물은
상세를 열 때마다 네이버를 부르는데, **그게 바로 캐시를 넣은 이유**입니다 —
정작 가장 필요한 경우에 캐시가 아무 일도 안 하게 됩니다.

[I219]에서 실거래에 쓴 처방과 같습니다. 못 찾으면 저장할 것이 없어 <b>상세를 열
때마다 12개월치를 다시</b> 받아 왔고, 그래서 `REFERENCE_MISS` 로
<b>"찾아봤지만 없었다"는 사실 자체</b>를 담아 뒀습니다.

#### 빈 목록이 두 가지 뜻이었다

```java
Optional<List<NewsArticle>> tryParse(...)

빈 Optional  →  못 읽었다   (담지 않는다)
빈 List      →  기사가 없다 (담는다)
```

둘 다 `List.of()` 로 두면 <b>구분이 사라집니다.</b> 처음에 `Parsed(usable, articles)`
레코드를 만들었다가 `Optional` 로 줄였습니다 — 같은 것을 말하는데 이름이 하나 적습니다.

#### 캐시가 죽어도 기사는 나온다

읽기·쓰기 실패는 <b>느려질 뿐</b>입니다. Redis 가 죽으면 그냥 네이버에 물어봅니다.

---

### I247. 상승·하락만 이름을 갖는다 · **[확정 — 구현됨]** · [I234] 조정

[I234]에서 판단 보류를 줄이려고 요인 다수결로 바꿨습니다. 그 결과 모달이
<b>"유지"를 결론으로 내세우게</b> 됐는데, 그건 <b>올라가지도 내려가지도 않는다고
단언하는 말</b>입니다.

**요인 다수결이 그쪽으로 기울었다는 것과, 그렇게 될 것이라 말하는 것은 다릅니다.**
...
<b>화면에서만 그렇게 부릅니다.</b> 도메인에서 `FLAT` 과 `UNCERTAIN` 을 합치면
사후 검증(구현 10)이 <b>"유지를 맞혔다"와 "판단을 안 했다"를 구분하지 못합니다.</b>

카드의 화살표(`▶`)도 그대로입니다. 바뀐 것은 <b>모달이 결론으로 내세우는 말</b>뿐입니다.

#### 고치다 라벨 표가 두 벌인 것을 찾았다

```js
const label = { UP: '상승', DOWN: '하락', FLAT: '횡보', UNCERTAIN: '판단 보류' };
                                          ^^^^ 서버는 '유지'로 바꿨는데 여기만 남았다
```

[I234]에서 `ForecastDirection.FLAT` 을 `유지`로 바꿨는데, 규칙 예측(`codeDirection`)의
라벨을 <b>화면이 따로 붙이고 있었습니다.</b> 그래서 "AI 모델은 <b>유지</b>를 예측했지만
규칙 기반 계산은 <b>횡보</b>였습니다" 처럼 <b>같은 방향을 두 이름으로</b> 부르고
있었습니다.

> 또 같은 모양입니다 — **규칙이 두 곳에 있으면 반드시 어긋납니다**
> ([I230]·[I232]·[I237]·[I239]·[I244]).

`DIRECTION_LABEL` 하나로 모으고, 서버와 같은 말을 쓰는지 테스트가 봅니다.

#### 이 변경은 <b>배포만으로 반영됩니다</b>

저장된 값을 읽어 그리는 것이라, [I234]와 달리 다시 분석할 필요가 없습니다.

---

### I248. 지표를 세어 방향을 정한다 · **[확정 — 구현됨]** · [I234] 대체

[I234]의 다수결이 <b>의도와 반대로</b> 돌고 있었습니다.

#### FLAT 이 모름의 하치장이었다

```java
// ForecastVerdictParser — 요인 하나가 'UNCERTAIN'일 수는 없다
return effect == UNCERTAIN ? FLAT : effect;
```

LLM 이 "모르겠다"고 해도 **`FLAT` 으로 바꿔** 넣습니다. 규칙 지표 여섯도 UP/DOWN/`FLAT`
만 냅니다 — 그래서 <b>요인에는 판단 보류가 아예 없습니다.</b>

그런데 `majorityOf` 는 `FLAT` 을 <b>진짜 한 표로</b> 셌습니다. **모르겠다는 표가
방향 표를 이깁니다.** "판단 보류를 줄이자"고 만든 규칙이 판단 보류를 만들고 있었습니다.
...
```
장기추세 ▲(보통) · 금리 ▼(보통) · 전세가율 ▼(보통) · 정비기대 ▲(낮음)

무게로   UP 3 : DOWN 4  →  하락
머릿수로 UP 2 : DOWN 2  →  동수 → 상승
```

**머릿수로 셉니다.** 무게를 반영하는 셈법은 규칙 예측(`codeDirection`)이 이미 하고
있고([I29]의 `MIN_MARGIN`), <b>둘이 같은 셈법이면 견줄 이유가 없습니다.</b>

#### 🤔 는 방향이 아니다

지표가 없거나 <b>전부 유지</b>면 카드에 `🤔 ▶` 를 띄웁니다.
"재료를 보고 판단을 안 한 것"과 "볼 재료가 없었던 것"은 사람에게 다른 이야기입니다.
색은 주지 않습니다 — 주면 그것부터 방향으로 읽힙니다([I150]).

---

### I249. AI 가 말하면 따르고, 안 말하면 센다 · **[확정 — 구현됨]**

[I248]의 셈법을 <b>언제</b> 쓰는가입니다.

```
AI 가 상승·하락을 말했다     →  그대로 따른다
AI 가 유지·판단 보류를 말했다  →  지표를 센다 (I248)
```

**유지도 "방향을 말한 것"이 아닙니다.** 규칙에서 유지 = 판단 보류로 합쳤고,
파서가 AI 의 "모르겠다"를 `FLAT` 으로 바꿔 넣으니([I247]) 그때도 우리가 셉니다.
...
```json
{ "direction": "UP | DOWN | FLAT | UNCERTAIN",   // 종합 결론
  "factors":   [{"name": "...", "effect": "UP", ...}] }   // 요인별 판단
```

**같은 응답 안의 두 필드**입니다. 그래서 <b>서로 어긋날 수 있습니다</b> —
요인에 ▲▼▼▲ 를 적어 놓고 결론은 판단 보류라고 쓰는 일이 실제로 있었습니다.

#### `(유력)`

AI 가 <b>방향을 말했고</b>, <b>지표를 세어도 같은 쪽</b>이면 결론 옆에 작게 붙입니다.
두 갈래가 <b>따로</b> 같은 결론에 닿은 것이라 더 믿을 만합니다.

> **AI 가 보류해 우리가 세어 넣은 경우는 유력이 아닙니다.** 그때 최종 결론과 지표
> 판정은 <b>같은 계산이라 반드시 같습니다</b> — 그것을 "일치"로 읽으면
> <b>보류였던 것이 전부 유력</b>이 됩니다. 이 기능에서 가장 조심할 자리입니다.

#### `llm_direction` 을 남긴다

`direction` 에는 <b>최종</b> 결론만 저장하고 있었습니다. 그래서 "AI 가 상승이라 했다"와
"AI 가 보류해 우리가 상승으로 정했다"가 <b>구분되지 않았습니다.</b>

```
direction       최종 결론
llm_direction   AI 가 스스로 낸 결론   ← 새로 남긴다
code_direction  규칙 기반 예측
factors         요인 (여기서 다시 셀 수 있다)
```

이력표(`price_forecast_history`)에도 같이 넣었습니다 — 사후 검증(구현 10)이
<b>누가 맞혔는지</b> 세려면 필요합니다. 옛 행은 `null` 이고, <b>모르는 것을 유력이라
하지 않습니다.</b>

#### 규칙 기반 계산을 <b>보류일 때도</b> 말한다

전에는 결론이 판단 보류면 참고 문구를 통째로 건너뛰었습니다.
**하필 그때가 가장 궁금한 순간입니다** — AI 가 못 정했으면 다른 셈은 뭐라 했는지
보고 싶습니다.

```
AI 모델은 판단을 보류했습니다. 규칙 기반 계산은 하락였습니다.
```

문구의 기준을 `direction` 에서 `llm_direction` 으로 옮겼습니다. 최종 결론으로 쓰면
<b>우리가 정한 것을 AI 가 말한 것처럼</b> 적게 됩니다.

---

### I250. 규칙을 고쳤는데 화면이 안 바뀌었다 · **[확정 — 구현됨]** · [I59] 조정

[I234]에서 다수결을 넣고, [I247]에서 표기를 고치고, [I248]에서 셈법을 다시 짜고,
[I249]에서 AI 우선을 넣었습니다. **화면은 그대로였습니다.**

```java
final ForecastVerdict verdict = forecast(gather(property));   // 새 규칙으로 계산한다
final String hash = hashToStore(verdict);
if (cached.isPresent() && hash.equals(cached.get().promptHash())) {
    return cached;        // ← 방금 계산한 것을 버리고 옛 값을 돌려준다
}
```

**새 판정을 계산해 놓고 버리고 있었습니다.**

#### 해시가 프롬프트만 보고 있었다

[I59]의 "같은 입력이면 다시 안 묻는다"는 <b>입력이 같아도 규칙이 바뀌면 다시 내야
한다</b>는 경우를 못 가립니다. 우리가 고친 것은 프롬프트가 아니라 <b>프롬프트 뒤의
판정 규칙</b>이었고, 지표가 그대로면 프롬프트도 그대로라 해시가 같았습니다.

```java
private static final String VERDICT_RULES_VERSION = "I249";

sha256(VERDICT_RULES_VERSION + "\n" + prompt.full())
```

**규칙을 고치면 이 값을 올립니다.** 채점이 `scoreVersion` 으로 푼 문제와 같습니다([I85]).

> 프롬프트를 고칠 때는 <b>안 올려도 됩니다</b> — 그건 해시가 이미 잡습니다.
> 올릴 때는 <b>지표를 읽는 방식이나 결론을 정하는 방식</b>이 바뀌었을 때입니다.

#### 유지도 다시 시킬 수 있어야 한다

`canTriggerForecast` 가 `UNCERTAIN` 만 열어 두고 있었습니다.
[I248]에서 <b>유지 = 판단 보류</b>로 합쳤으니 둘 다 열어야 합니다 —
하나만 열어 두면 <b>같은 상태인데 어떤 것은 눌리고 어떤 것은 안 눌립니다.</b>

상승·하락은 여전히 못 누릅니다. **그건 답입니다.**

---

### I251. 국토부는 오류도 200으로 준다 · **[확정 — 구현됨]** · [I140] 보완

전망 모달은 "인근 실거래 비교 자료가 없습니다"라는데 <b>실거래 모달에는 8건이
버젓이</b> 떠 있었습니다.
...
이 본문에는 `<item>` 이 없어 파서가 <b>빈 목록</b>을 돌려주고, 수집기는 그것을
<b>"그 달은 거래가 없었다"</b>로 저장합니다. 과거 달은 다시 받지 않으므로([I128])
<b>영영 구멍</b>이 됩니다.

> [I140]에서 "실패를 캐시에 굳히지 않는다"고 고쳤는데 <b>연결 실패만</b> 막았습니다.
> **429는 연결이 되고 200이 옵니다** — 그 그물을 그냥 통과했습니다.

`resultCode` 를 봅니다. 정상(`00`·`INFO-000`)이 아니면 <b>못 받은 것</b>으로 보고
저장하지 않습니다. **코드가 아예 없으면 통과**시킵니다 — 없다고 실패로 몰면
멀쩡한 달까지 안 받게 됩니다.

> 이미 굳은 행은 코드로 못 고칩니다.
> `DELETE FROM monthly_trade_cache WHERE trade_count = 0;` 를 한 번 돌려야 합니다.

---

### I258. 정상 코드를 추측해서 <b>정상 응답을 전부 버렸다</b> · **[확정 — 구현됨]** · [I251] 긴급 수정

```
WARN Ministry trades rejected - not storing this month.
     lawdCd=11290, dealYmd=202608, resultCode=000, resultMsg=OK
```

**`000` 은 정상입니다.** [I251]에서 정상 코드를 이렇게 잡았습니다.

```java
Set.of("00", "0", "INFO-000")   // ← 000 이 없다
```

**실물을 안 보고 이름을 추측했습니다.** 그 결과 배포된 순간부터 <b>모든 정상 응답을
버렸고</b>, 실거래가 하나도 안 쌓였습니다. 오류를 0건으로 굳히지 말자고 넣은 그물이
<b>정상까지 통째로</b> 걷어 냈습니다.

#### 이름을 세지 말고 모양을 본다

```java
Pattern.compile("^(?:INFO-)?0+$")   // 0 · 00 · 000 · 0000 · INFO-000
```

열거하지 않고 <b>0만으로 이뤄졌는가</b>를 봅니다.

#### 그래도 또 틀릴 수 있으니

코드 이름을 못 맞혀도 <b>본문이 왔으면 정상</b>으로 봅니다.

```java
if (TOTAL_COUNT.matcher(xml).find()) return false;   // body 가 있다 = 진짜 응답이다
```

오류 응답에는 `body` 가 없어 `totalCount` 도 없습니다. **한 번 틀린 방식(이름 열거)에
기대지 않으려는 것**입니다.

#### 테스트가 그물에 걸려 있었다

처음 쓴 테스트가 `totalCount` 가 있는 본문을 썼습니다. 그러면 <b>코드 규칙을 지워도
통과합니다</b> — 돌연변이가 살아남아 알았습니다. `totalCount` 를 뺀 본문으로 고쳐
코드 규칙만 재게 했습니다.

> **두 그물이 서로를 가립니다.** 겹쳐 두는 건 좋지만, 그러면 <b>각각을 따로
> 재야</b> 합니다.

---

### I259·I260·I261. 화면이 <b>모르는 것을 안다고 했다</b> · **[확정 — 구현됨]**

세 가지가 같은 모양입니다 — <b>아직 안 온 것</b>을 <b>없는 것</b>으로 말했습니다.

#### I259. 받아 오는 중인데 "없습니다"

상세 모달이 실거래를 <b>배경에서</b> 받아 옵니다([I106]). 그동안 `fetched=0` 을
돌려주었더니 화면이 그걸 이렇게 읽었습니다.

```
최근 12개월 동안 같은 단지·면적의 거래내역이 없습니다.
```

**새로고침해야 채워졌습니다.** `looking` 을 따로 두어 <b>진행 막대</b>를 띄우고,
다 받으면 스스로 채웁니다.

#### I260. 왜 안 맞았는지 실물을 남긴다

```
fetched=4484, nameMatched=0
```

이 로그로는 원인을 못 짚습니다 — 이름이 다른 건지, 동·번지가 안 온 건지,
우리 주소를 못 읽은 건지. 우리가 읽은 주소와 국토부가 준 거래 몇 건의
<b>이름·동·번지</b>를 같이 남깁니다.

#### I261. 채점이 끝났는데 "분석하고 있습니다"

배너는 `detailItem.scores` 가 비었는지만 봅니다. 그 값은 <b>목록에서 온 것</b>이라,
목록이 갱신되고 `syncDetailItem` 이 돌아야만 걷힙니다.

> **사슬 중 하나만 끊겨도 영영 남습니다.** 채점이 AI까지 다 끝났는데도 배너가
> 계속 떠 있는 것을 실제로 겪었습니다.

<b>어느 링크가 끊겼는지는 아직 모릅니다.</b> 상세가 <b>스스로</b> 다시 묻게 했습니다 —
사슬을 타지 않으므로 어느 쪽이든 듣습니다.

#### 같은 처방을 세 번째 씁니다

[I72]에서 AI 추천에 넣은 것과 같습니다. **멈추는 조건을 넷 다** 둡니다 —
채워짐 · 모달 닫힘 · 다른 매물 · 시도 상한. 하나라도 빠지면 탭이 열려 있는 동안
계속 두드립니다.

#### 테스트를 두 번 고쳤다

처음 짠 것이 <b>폴링을 직접 불러서</b> "상세를 열면 시작한다"를 못 쟀고,
멈춤을 <b>횟수로만 봐서</b> "멈춘다"도 못 쟀습니다 — 3초 간격이라 안 멈춰도
같은 횟수가 나왔습니다. 시계를 직접 보고, 여는 함수를 거치도록 고쳤습니다.

---

### I262. "받아 오는 중"이 <b>한 시간을 돌았다</b> · **[확정 — 구현됨]** · [I259] 수정

[I259]가 만든 결함입니다. 배포 뒤 새 매물의 실거래 프로그래스바가 멈추지 않았습니다.

```java
final List<ReferenceTransaction> stored = findByPropertyId(propertyId);
if (!stored.isEmpty()) { return toCard(...); }
gate.runAll(...);                            // ← 기다리는 함수다
return ReferenceCardResponse.looking(...);   // ← 저장된 게 없으면 무조건 "받아 오는 중"
```
...
`looking` 을 <b>"저장된 행이 없다"</b>로 판단했습니다. 그런데 못 찾은 매물은
저장될 것이 <b>영영 없습니다.</b> 조건이 참인 채로 고정됩니다.

찾아봤다는 사실은 이미 `REFERENCE_MISS` 에 남아 있었습니다 ([I219]) —
<b>이 길만 그것을 안 봤을 뿐입니다.</b>

#### ② 끝났는데 자국을 안 남기는 길이 있었다

법정동코드를 못 만들면 국토부를 부르기도 전에 되돌아서는데, 그 길은
`REFERENCE_MISS` 를 안 남겼습니다. ①을 고쳐도 <b>이 길로 들어오면 여전히 안 멈춥니다.</b>

> **모든 종료 경로가 자국을 남겨야 합니다.** 하나라도 조용히 빠져나가면
> "끝났는가"를 묻는 쪽은 영원히 답을 못 얻습니다.

다만 이건 자료가 없는 게 아니라 <b>물어볼 주소를 못 만든 것</b>이라 10분만
기억합니다. 사전이 채워지면 곧 풀려야 합니다.

#### ③ 기다리지 않겠다면서 기다리는 함수를 불렀다

`runAll` 은 이름 그대로 <b>전부 끝날 때까지 붙잡습니다.</b> [I259]의 주석은
"화면은 기다리지 않는다"라고 써 놓고 그 함수를 불렀습니다. 요청 하나가
12개월치를 다 받을 때까지 붙잡혀 있다가, 그러고 나서 "받아 오는 중입니다"라고
답한 셈입니다.

`VirtualThreadGate.detach` 를 새로 만들었습니다 — 자리는 똑같이 세마포어로
묶되 <b>결과를 안 기다립니다.</b> 테스트에서 이 차이가 **120초**로 나왔습니다.

#### ④ 다시 묻는 화면이 조회를 매번 새로 띄웠다

3초마다 묻는 화면 하나가 1분에 <b>스무 벌</b>을 띄웠습니다. 각각 12개월치이니
국토부에 240번입니다. `REFERENCE_LOOKING` 표시를 세워 <b>도는 것이 있으면
기다리기만</b> 합니다. 표시에는 TTL(3분)을 둡니다 — 프로세스가 죽어 못 지웠을 때
아무도 다시 못 띄우는 일을 막습니다.

#### 화면도 거짓말을 했다

폴링이 스무 번을 채우면 <b>조용히 멈추기만</b> 했습니다. `looking` 은 참인 채라
막대가 계속 돕니다. **그만 물으면 그만 돈다고 말해야 합니다.**

그리고 <b>지쳐서 멈춘 것</b>과 <b>정말 없는 것</b>을 같은 문장으로 말하면 안 됩니다 —
"없습니다"가 거짓이 됩니다. 따로 띄우고 "지금 다시 확인"을 붙였습니다.

#### 돌연변이 둘이 살아남았다

처음 짠 테스트는 ②와 ④를 못 쟀습니다. ④를 재려던 테스트는 <b>MISS 검사가 먼저
걸려</b> 그 줄까지 가지도 않았고, ②는 <b>테스트가 법정동코드를 심어 놔서</b>
그 길을 안 지났습니다. 국토부 응답을 붙잡아 두는 스텁으로 <b>진짜 도는 중</b>을
만들고, 사전에 없는 동을 쓰는 테스트를 따로 더했습니다.

---


### I263. 임장 경로 계산이 <b>504</b>가 났다 · **[확정 — 구현됨]**

「경로 계산」을 누르면 아무 일도 안 일어났습니다. 콘솔에는 이렇게 있었습니다.

```
POST /api/itinerary/optimize   504 (Gateway Timeout)
```

#### 구간은 매물 수의 제곱이다

매물 다섯이면 **25쌍**, 일곱이면 **49쌍**입니다. 그것을 <b>한 줄로</b> 물었습니다.

| 모드 | 예전 | 왜 |
|---|---|---|
| 대중교통 | 미리 받되 **순차** | `findTransitBatch` 가 for 문이었다 |
| 자동차 | **미리 받지도 않음** | 행렬 람다가 부를 때마다 카카오를 불렀다 |

카카오 읽기 제한이 6초, ODsay 가 15초입니다. 느려진 날 한 줄로 돌면
**분 단위**가 되고, 프록시는 60초에 끊습니다.

#### 자동차는 담아 두지도 않았다

대중교통은 [I176]에서 `transitMemo` 를 두어 <b>행렬과 구간 안내가 같은 답을
나눠 쓰게</b> 했습니다. **자동차만 빠져 있었습니다** — 같은 길을 두 번 물었습니다.

#### 고친 것

- `VirtualThreadGate.runWithin` — 정해진 시간 안에 끝나는 만큼만 돌리고 <b>남은 것은 버립니다</b>
- `itineraryGate` (상한 8) — 빨리 하자고 넓히면 429 를 다 같이 받습니다
- 자동차도 미리 받고(`prewarmDriving`) `driveMemo` 에 담습니다
- ODsay 가 시간 안에 못 준 구간은 <b>LLM 추정으로 넘깁니다</b> — [I210]이 이미 만들어 둔 길입니다

#### 담아 두기가 [I196]을 지웠다

처음 짠 열쇠는 **좌표 넷**뿐이었습니다. 그러자 세 번째 매물이 <b>09시의 길</b>을
썼습니다 — 실제로는 13시에 출발하는데도요. 기존 테스트가 잡았습니다.

> **담아 두기는 "같은 물음"의 정의를 바꿉니다.** 시각을 빼면 시각별로 묻게
> 해 둔 [I196]이 통째로 무의미해집니다.

열쇠에 출발 시각을 넣었습니다. 그래서 미리 받아 두기는 <b>행렬에만</b> 듣습니다 —
순서를 정할 때는 모두 같은 시각을 쓰기 때문입니다. 구간 안내는 다시 묻지만
`n-1`번이라 감당할 만합니다.

---

### I264. 화면이 <b>없는 함수</b>를 부르고 있었다 · **[확정 — 구현됨]**

매물 카드마다 콘솔에 빨간 줄이 났습니다.

```
Alpine Expression Error: scoredComfort is not defined
Expression: "{mine: scoredComfort(r)}"
```

배지 색이 <b>늘 남의 것</b>으로 보였는데, **아무 테스트도 안 깨졌습니다.**
빌드 단계가 없어 Alpine 식은 브라우저에서만 드러납니다.

같은 규칙이 `visitedTitle` 안에 <b>또 한 벌</b> 있었습니다 — 그쪽은 멀쩡히 돌고
있었고요. 함수를 만들어 양쪽이 그것 하나를 보게 했습니다.

#### 계열 전체를 잡는 검사를 뒀다

`index.mustache` 의 `x-…` · `:…` · `@…` 식에서 <b>부르는 이름</b>을 모두 뽑아
`app.js` 에 있는지 봅니다. 지금 **188개를 부르고 464개가 정의**되어 있으며,
못 찾은 것은 `$nextTick`(Alpine 매직) 하나뿐이었습니다.

> 이 검사는 **0건 통과**를 막으려고 "정의를 100개 이상 찾았는가"를 먼저 봅니다.
> 정규식이 어긋나면 아무것도 못 찾고도 통과하기 때문입니다.

---

### I270. 화면이 <b>999분</b>을 이동시간이라고 말했다 · **[확정 — 구현됨]** · [I263] 후속
...
카카오 길찾기 <b>하루 한도가 끝났습니다.</b> 그런데 화면은 이렇게 말했습니다.

```
예상 이동시간 합계: 3996분
1. 상계주공7단지   06:39 (+1일) 도착      약 999분
2. 석관신동아파밀리에 23:43 (+1일) 도착      약 999분
```

**999는 "모른다"는 뜻의 내부 값**이었습니다([I176]의 `UNREACHABLE_MINUTES`).
그것이 그대로 화면까지 나가 <b>숫자로 읽혔습니다.</b> 합계 3996분은 지어낸 값이고,
도착 시각 `(+1일)` 은 999분을 세 번 더한 결과입니다.

> 이 저장소가 되풀이하는 그것입니다 — **모르는 것이 낙관적인 방향으로 조용히
> 값이 되는 것.** [I219]·[I259]·[I262]가 모두 같은 모양이었습니다.

#### 모르는 것은 null 이다

`ItineraryLegResponse.minutes` 를 <b>nullable</b> 로 바꿨습니다.

- 구간: `이동시간 미확인`
- 합계: <b>아는 것만</b> 더하고, 못 받은 구간 수(`unknownLegs`)를 함께 보냅니다
- 도착 시각: 앞 구간에 모르는 것이 하나라도 있으면 <b>안 보여 줍니다</b>
- 출발 시각도 <b>못 미룹니다</b> — 999분을 더해 뒤 구간을 하루 뒤의 길로 묻고 있었습니다

#### 하루치를 다 쓴 것을 기억한다

한 번 한도를 보면 <b>그날은 더 부르지 않습니다.</b> 안 그러면 「경로 계산」을 누를
때마다 49번을 더 던져 <b>얻는 것 없이</b> 차단기만 여닫습니다. [I210]이 ODsay 에
쓴 것과 같은 모양입니다.

**차단기가 열린 것은 한도가 아닙니다.** `CallNotPermittedException` 은 <b>결과</b>이지
원인이 아니라, 그걸로 하루를 접으면 일시적인 장애 하나에 그날을 통째로 버립니다.
원인 사슬에 실제로 한도라는 말이 있을 때만 표시합니다.

#### 내가 한도를 더 빨리 태웠다

[I263]에서 49쌍을 <b>동시에</b> 부르게 했습니다. 504는 사라졌지만 한 번 누를 때마다
49건이 한꺼번에 나가고, 자동차 길은 <b>요청 안에서만</b> 기억하므로([I263]의
`driveMemo`) 다시 누르면 또 49건입니다. 한도가 있는 API에는 <b>그 자체가 문제</b>입니다.

---

### I271. 한 번의 실패가 <b>몇 분짜리 폭풍</b>이 됐다 · **[확정 — 구현됨]** · [I210]·[I218] 수정

같은 날 대중교통도 무너졌습니다.

```
04:51  ODsay 429 (하루 한도) → LLM 대체, 9구간
04:52  Claude read timeout(60초) → 재시도 → 또 timeout → answered 0 of 9   ← 2분
04:54  차단기 OPEN → 구간마다 1건씩 → "LLM is busy, waiting 2000ms" → 실패 → 반복
       … 04:55 까지 같은 줄이 계속
```

**셋이 겹쳤습니다.**

#### ① 차단기가 열린 것을 "붐빈다"로 읽었다
...
**열린 차단기에 다시 던지면 절대 성공하지 않습니다.** 그런데 어댑터가 모든 실패를
{@code "call failed"} 하나로 뭉개서, 부르는 쪽은 <b>"붐빈다"와 "차단됐다"를 구별하지
못했습니다.</b> [I218]의 "한 번만 더"가 그 자리에서 <b>영원한 되풀이</b>가 됐습니다.

`LlmAvailability` 를 뒀습니다 — [I270]의 `DirectionsQuota` 와 같은 모양입니다.
차단기가 스스로 반쯤 열어 볼 때까지(설정과 같은 60초) 묻지 않습니다.

> **붐빔까지 안 물으면 안 됩니다.** 일시적인 실패 하나로 추정을 통째로 버리게 됩니다 —
> [I218]이 그걸 막으려고 넣은 것입니다. 구별해야 둘 다 산다는 뜻입니다.

#### ② 못 받은 것을 기억하지 않았다

미리 받아 두기가 <b>답을 낸 것만</b> 기억에 담았습니다([I210]의 "못 낸 열쇠는 빠집니다").
그래서 뒤 단계가 그 자리를 <b>다시 물었습니다</b> — 한 번의 실패가 구간 수만큼 늘어납니다.

**못 받은 것도 기억합니다.** 그것도 결과입니다 — [I219]에서 배운 그대로입니다.

#### ③ 행렬이 기억을 안 봤다

가장 큰 것이었습니다. `travelTime` 이 <b>캐시만</b> 보고 기억을 안 봤습니다.
최적화기는 같은 쌍을 여러 번 물어보므로, 매물 <b>넷</b>에 **52번**이 나갔습니다.

```
[I176] 의 주석: "행렬을 만들며 이미 받은 것을 그대로 쓴다"
실제:            행렬은 그 기억을 읽지 않았다 — 쓰기만 했다
```

<b>주석이 하려던 일을 코드가 안 하고 있었습니다.</b>

#### 화면이 무엇을 보게 되나

[I270]과 합쳐, 이제 이런 날에는 <b>몇 초 안에</b> 답이 오고 구간마다
`이동시간 미확인` 이라고 적힙니다. 합계에는 안 들어가고, 그 사실을 함께 말합니다.

---

### I272. 한 번 받은 길은 <b>다시 안 받는다</b> · **[확정 — 구현됨]** · [I270] 후속

매물 일곱이면 한 번 계산에 <b>49쌍</b>입니다. 그런데 자가용 길은 <b>담아 두는 곳이
아무 데도 없었습니다</b> — [I263]의 `driveMemo` 는 요청 안에서만 삽니다. 「경로 계산」을
누를 때마다 49건이 나갔고, 한도가 있는 API 는 그것으로 하루가 끝났습니다([I270]).

#### 열쇠에 출발 시각을 넣는다

시각을 빼면 화요일 14시와 일요일 14시가 <b>같은 길</b>이 되어 [I196]이 무의미해집니다.
[I263]에서 요청 안의 기억에 같은 실수를 했다가 기존 시험에 걸렸습니다 — 두 번째입니다.

#### 못 받은 것은 담지 않는다

한 번의 실패를 <b>하루 동안</b> 물려주게 됩니다. [I267]에서 모델 목록에 같은 판단을
했습니다.

#### 담아 두기가 공급자보다 위에 있다

`KakaoDirectionsAdapter` 는 <b>포트를 직접 구현하지 않습니다.</b> `CachingDirections`
가 포트이고 그것을 감쌉니다 — `TransitWithLlmFallback` 이 ODsay 를 감싸는 것과 같은
모양입니다. 나중에 다른 길찾기를 붙여도([ROUTE_FALLBACK.md]) 담아 두기는 그대로입니다.

> 둘 다 포트이면 시험이 갈아 끼우는 <b>대역과 우선 빈이 부딪힙니다.</b>

---

### I273. 한도가 끝나도 <b>서비스는 이어진다</b> · **[계획 — 미구현]**

설계는 **[docs/ROUTE_FALLBACK.md](./ROUTE_FALLBACK.md)** 에 따로 두었습니다.
카카오·ODsay 하루 한도가 같은 날 함께 끝나면서 임장 플래너가 통째로 멈춘 일
([I270]·[I271])의 <b>근본 대책</b>입니다.

```
① 한도를 미리 안다  (Redis 잠금, 자정에 저절로 풀림)
② TMAP 으로 넘어간다
③ LLM 에게 동선을 묻는다  (지도는 직선, 안내는 LLM 의 말)
④ 그래도 안 되면 <b>내일 다시</b> 라고 말한다
```

---

### I274. 하나도 못 받았으면 <b>결과가 아니다</b> · **[확정 — 구현됨]** · [I270] 후속

[I270]에서 999를 걷어냈더니 이번엔 이렇게 나왔습니다.

```
출발지 → 석관신동아파밀리에      이동시간 미확인
  🚗 약 null분
```

**같은 값을 두 곳에서 그리고 있었습니다.** 한 곳은 고쳤는데 "자가용인데 도로를
못 받았으면 시간만이라도" 하는 줄이 남아 있었습니다. `null` 을 그대로 이었습니다.

#### 그런데 더 큰 문제가 있었다

구간을 <b>하나도</b> 못 받았는데도 **순서를 늘어놓고 있었습니다.** 그 순서는
최적화기가 <b>전부 같은 값(못 감)</b>으로 매긴 것이라 <b>아무 근거가 없습니다.</b>
그걸 보여 주면 사람은 <b>계산된 동선</b>으로 읽습니다.

**일부만 못 받은 것과 다릅니다.** 일부라면 나머지는 쓸 만하니 합계에서 빠졌다고
적어 두면 됩니다([I270]). 전부라면 **말을 해야 합니다.**
...
화면이 `unknownLegs === legs.length` 를 스스로 세게 두면 <b>규칙이 두 벌</b>이
됩니다 — 이 저장소가 그때마다 갈렸습니다([I265]·[I269]). 응답에 `status` 와
`message` 를 실어 <b>화면은 따르기만</b> 합니다. 그래야 시험으로도 잡힙니다.

> 문구도 서버가 줍니다. 화면에 또 적으면 <b>둘이 어긋날 자리</b>가 생깁니다.

#### 모달을 닫는 목록에도 넣어야 한다

`closeTopModal()` 의 목록에 없으면 <b>배경을 눌렀을 때 엉뚱한 모달이 닫힙니다.</b>
[I244]에서 만든 구조라 새 모달마다 등록이 필요합니다.

이것은 [ROUTE_FALLBACK.md] 사다리의 <b>④</b> 입니다. ①②③ 은 아직입니다.
...
### I267. 자리마다 <b>모델을 따로 고른다</b> · **[확정 — 구현됨]** · [I58] 확장

지금까지는 환경변수 하나(`LLM_CLAUDE_MODEL`)가 전부라 <b>바꾸려면 배포</b>해야 했습니다.
그런데 AI를 쓰는 자리는 넷이고 성격이 다릅니다.

| 자리 | 하는 일 | 설정 열쇠 |
|---|---|---|
| AI 추천도 | 매물 하나를 읽고 점수와 이유를 낸다 | `llm.model.recommendation` |
| 비교 우위 분석 | 매물 전체를 한 번에 견준다 | `llm.model.comparative` |
| 가격 전망 | 실거래 추세를 읽는다 | `llm.model.forecast` |
| 직주근접 추정 | ODsay 가 막혔을 때 ([I210]) | `llm.model.commute` |

하나로 묶어 두면 <b>가장 비싼 자리에 맞춰야</b> 합니다.

#### `system_config` 에 둔다

이미 관리자 설정이 쓰는 표입니다. 새 표를 만들면 <b>설정이 두 군데</b>가 됩니다.

<b>씨앗 값은 비워 둡니다.</b> 여기에 지금의 기본 모델을 박아 두면, 나중에 배포로
기본을 바꿔도 <b>DB에 남은 옛 이름이 이깁니다.</b> 비어 있으면 `llm.claude.model` 을 씁니다.

#### 목록은 물어서 채운다

손으로 적어 두면 늙습니다 — 모델이 바뀔 때마다 배포해야 하고, <b>이미 없어진 이름</b>을
고를 수 있게 됩니다. Anthropic 의 `/v1/models` 를 부르고 <b>48시간</b> 담아 둡니다.

**실패는 담지 않습니다.** 빈 목록을 담으면 한 번의 실패를 <b>이틀 동안</b> 물려줍니다.

못 받았을 때 화면은 <b>지금 쓰는 모델만</b> 보여 줍니다 — 빈 드롭다운을 주면 고칠 수도
없고, 지어낸 이름을 채우면 그 자리의 AI가 조용히 죽습니다.

#### 아는 열쇠·아는 모델만 받는다

- 임의의 설정 키를 이 길로 고칠 수 있으면 <b>규제 파라미터나 슬랙 스위치까지</b> 바뀝니다
- 모델 이름이 목록 밖이면 400이 돌아오는데, 그 실패는 <b>"AI가 답을 안 줬다"</b>로 묻힙니다

#### 자유 입력칸에 섞지 않았다

일반 설정 표에 함께 두면 <b>텍스트 칸</b>이 되어 아무 문자열이나 들어갑니다.
`LLM` 갈래는 그 표에서 빼고 <b>드롭다운이 있는 층</b>을 따로 두었습니다.

#### 씨앗을 심는 방식이 틀려 있었다

`SystemConfigBootstrap` 은 <b>표가 비었을 때만</b> 심었습니다. 그래서 이미 돌고 있는
곳에는 <b>새 설정이 영영 안 들어갑니다</b> — 항목을 더해도 운영에서는 안 보입니다.
<b>키 하나씩</b> 보도록 고쳤습니다. 이미 있는 것은 건드리지 않으므로 사람이 고른 값을
덮지 않습니다.

#### 모델 목록은 Feign 클라이언트를 따로 두었다

`ClaudeFeignClient` 에 메서드를 하나 더하니 <b>함수형 인터페이스가 아니게 되어</b>
대역을 람다로 만들던 시험 다섯 자리가 깨졌습니다. 무엇보다 성격이 다릅니다 —
하나는 매번 부르는 생성이고 하나는 이틀에 한 번 부르는 조회입니다.
...
### I278. AI 모델 설정 — 여백·요청 로그·Fable 표시 · **[확정 — 구현됨]**

배포 후 확인하다 나온 세 가지를 마저 손봤습니다.
...
네 자리(추천도·비교 우위·가격 전망·직주근접 추정) 모두 "LLM에 물었다"는 로그는
있었지만 <b>어느 모델로</b> 물었는지는 없었습니다. `Asking LLM for ...` 로그마다
`model={}` 를 더했습니다 — 비교 우위 분석에는 그 로그 자체가 아예 없어서 새로
만들었습니다.

#### Fable 은 API에 표시가 없다

Anthropic `/v1/models` 를 실제로 불러 봤습니다. 필드는 `type`·`id`·`display_name`·
`created_at`·`max_input_tokens`·`max_tokens`·`capabilities` 뿐이고, <b>"이건 특수
모델"이라고 가리키는 필드는 없습니다.</b> 유일한 신호는 `id` 가 `claude-fable-` 로
시작한다는 것뿐입니다.

`LlmModelOption` 에 `special` 을 더하고, `LlmModelOption.of(id, displayName)`
팩토리가 id 접두어로 스스로 판정합니다 — 부르는 쪽마다 규칙을 되풀이하지 않도록
한 곳에 모았습니다. 드롭다운은 특수 모델 이름 뒤에 `*` 를 붙이고, 절 설명에
그 뜻을 한 줄 적었습니다.

> **필드가 없다는 것도 답입니다.** 있는 척 지어내지 않고, 실제로 물어본 결과를
> 그대로 전합니다 — 이름 규칙으로 가리는 것이 지금 할 수 있는 최선입니다.
...
### I280. 저장은 됐는데 화면만 "기본 모델" 이었다 · **[확정 — 실물로 드러난 버그]** · [I112]·[I267] 후속

모델을 골라 저장하고 설정을 다시 열면 자리 넷이 전부 <b>"기본 모델"</b>로 보였습니다.

#### 저장은 멀쩡했습니다

먼저 의심한 곳부터 지웠습니다. `system_config` 에 얹히는 구조라 새 테이블은 없고
(`DDL-repair.sql` 을 다시 돌릴 일도 없습니다), 캐시도 저장할 때마다 지우고 있었습니다.
`PUT /api/admin/llm-models` → `GET` 왕복을 시험으로 묶어 확인했습니다 — <b>고른 값은
그대로 저장되고 그대로 돌아옵니다.</b> 화면만 거짓말을 하고 있었습니다.

#### `x-model` 하나로는 `<select>` 를 못 채웁니다

vendored Alpine 을 jsdom 에 올려 실제로 재현했습니다.

```
처음 모달을 열었을 때
  화면에 보이는 것 : "기본 모델"
  select.value     : ""
  Alpine 이 쥔 값  : "claude-sonnet-5"   ← 값은 멀쩡히 들고 있다
```

`<option>` 은 `x-for` 로 <b>나중에</b> 붙습니다. 그전에 `x-model` 이 값을 넣으면 맞는
항목이 없어 `selectedIndex` 가 −1 이 되고, 뒤이어 옵션이 붙는 순간 브라우저가
<b>첫 항목을 고릅니다</b> — 그게 "기본 모델" 이었습니다. 상태와 화면이 어긋난 것이지
저장이 틀린 게 아닙니다.

`:selected` 를 함께 걸어 고쳤습니다. 후보 넷을 같은 조건에서 견주어 골랐고,
`:selected` 를 다시 떼면 신고받은 화면이 그대로 재현됩니다.

#### [I112]를 안 지키고 있었습니다

`closeSettings()` 가 `showSettings` 와 `error` 만 껐습니다. <b>"모달은 열 때 다시 읽고
닫을 때 비운다"</b>([I112])를 어긴 것입니다. 설정 모달이 쥔 것 전부(설정·AI 모델·
알림·규제 파라미터·규제지역)를 닫을 때 비웁니다 — 모달 밖에서 읽는 곳이 없음을
확인했습니다.

> **비우기만 했다면 버그가 더 커졌을 겁니다.** 닫을 때 비우면 `x-if` 가 헐렸다 다시
> 서므로 "처음 그리기"가 <b>열 때마다</b> 일어납니다. 위의 `:selected` 를 함께 고치지
> 않으면 지금까지 가끔 나던 증상이 <b>항상</b> 나게 됩니다. 실측으로 확인했습니다.

#### 화면과 상태가 어긋날 자리를 하나 더 막았습니다

- 저장 뒤 `llmModels` 만 갈아 끼우고 `llmForm` 은 그대로 뒀던 것을, <b>읽을 때와 같은
  길</b>(`applyLlmModels`)로 모았습니다 — 보낸 값과 저장된 값이 다를 수 있습니다.
- 목록에 없는 모델이 저장돼 있으면(Anthropic 이 내린 모델) 비웁니다. 그대로 두면
  드롭다운은 "기본 모델"을 보여 주는데 상태에는 그 모델이 남아, 저장할 때 <b>고른 적도
  없는 값</b> 때문에 "쓸 수 없는 모델입니다"가 떴습니다.

화면 동작은 Java 로 재현할 수 없어, `:selected` 표시가 템플릿에 남아 있는지만
`ViewControllerTest` 로 지킵니다. 지우면 같은 버그가 조용히 돌아옵니다.

---

### I281. 누르지도 않은 버튼이 같이 깜빡였다 · **[확정 — 실물로 드러난 버그]** · [I115] 후속

"AI 모델 저장"을 눌렀는데 그 위의 <b>"설정 저장"이 함께 깜빡였습니다.</b>

저장 버튼이 전부 `:disabled="loading"` 로 <b>전역 플래그 하나</b>에 묶여 있었습니다
(화면 통틀어 29개). 하나를 누르면 나머지가 전부 잠겼다 풀리고, 응답이 빠르면
그 왕복이 <b>깜빡임</b>으로 보입니다. [I115]에서 진행 막대에 대해 이미 같은 것을
겪었습니다 — "빠른 응답에도 켜면 번쩍였다 사라져 오히려 어수선하다".

`savingKey` 를 두어 <b>누른 버튼만</b> 잠급니다. 설정 모달의 저장 버튼 셋
(설정·AI 모델·규제 파라미터)에만 적용했습니다 — 서로 다른 API 라 하나가 도는 동안
다른 것을 눌러도 될 일이고, 이중 제출은 자기 버튼이 잠기는 것으로 막힙니다.

> **나머지 26개는 그대로 뒀습니다.** 전역 `loading` 은 진행 막대·업로드 표시에도
> 걸려 있어 한꺼번에 걷어내면 건드릴 화면이 너무 넓습니다. 신고된 자리를 고치고,
> 같은 증상이 다른 화면에서 나오면 그때 같은 방식으로 옮깁니다.

vendored Alpine 을 jsdom 에 올려 확인했습니다 — 되돌리면 신고받은 대로
"설정 저장"이 같이 꺼집니다. 템플릿에 `savingKey` 가 남아 있는지는
`ViewControllerTest` 로 지킵니다.

---

### I282. 숨긴 뒤에 되돌린 스크롤은 버려진다 · **[확정 — 실물로 드러난 버그]** · [I183] 정정

설정 모달을 열면 <b>맨 위가 아니라 중간(AI 모델 절)부터</b> 보였습니다.
[I183]에서 이미 고쳤다고 적어 둔 바로 그 증상입니다.

#### 고친 방법이 틀렸습니다

[I183]은 <b>닫힐 때</b> 되돌렸습니다 — `style` 이 `display:none` 으로 바뀌는 것을
`MutationObserver` 로 지켜보다가 `.modal-card` 의 `scrollTop` 을 0 으로 넣었습니다.
그런데 그때는 <b>이미 안 그려진 상태</b>입니다. 실제 브라우저로 재 봤습니다.

```
열린 채 내린 스크롤        = 500
숨긴 채 0 을 넣고 읽으면   = 0      ← 넣은 것처럼 보인다
다시 열었을 때 실제 위치   = 500    ← 그런데 그대로다
```

**안 그려진 요소에 넣은 `scrollTop` 은 버려집니다.** 읽으면 0 이 나와서 된 것처럼
보이는 것이 [I183]이 통과한 것처럼 보인 이유입니다.

#### 열릴 때 되돌립니다

같은 관찰자를 <b>열리는 쪽</b>으로 뒤집었습니다(`watchModalOpen`). 그려진 뒤에
넣으므로 먹습니다. 세 가지를 지킵니다.

- <b>열림으로 바뀔 때만</b> 되돌립니다. 열려 있는 동안 다른 `style` 이 바뀌었다고
  되돌리면 <b>보던 자리가 맨 위로 튑니다.</b>
- <b>열린 그 모달만</b> 되돌립니다 — 겹쳐 뜬 모달([I122])에서 아래 것까지 건드리지
  않습니다.
- 닫을 때는 아무것도 하지 않습니다. 어차피 버려지는 일이었습니다.

`closeAllModals()` 의 `resetModalScroll()` 은 그대로 뒀습니다 — 거기서는 Alpine 이
아직 숨기기 <b>전</b>이라 실제로 먹습니다.

> **읽어서 0 이 나온다고 들어간 것이 아닙니다.** 화면 코드는 "넣었다"까지가 아니라
> <b>"다시 열었을 때 어디에 있나"</b>까지 봐야 확인한 것입니다.


---

### I265. 동/호가 <b>저장되지 않았다</b> · **[확정 — 구현됨]**

붙여넣기 화면에는 `102동`이 멀쩡히 떴는데 DB의 `dong_ho` 는 늘 `NULL` 이었습니다.

```js
buildPasteRequest() {
    return {
        name: value('name'),
        dealType: dealCode,      // ← dongHo 가 없다
```

파서는 읽고([DongHoExtractor]), 화면은 보여 주고, 사람은 「확정」을 눌렀는데
<b>payload 를 만드는 자리에서 빠졌습니다.</b> 이 저장소에서 되풀이되는 모양입니다 —
[I219]·[I233]·[I262]가 모두 "이미 있는 값을 안 쓰고 있었다"였습니다.

#### 이름이 셋이었다

목록·지도는 단지명만, 상세 모달만 동·호를 붙였습니다. 같은 규칙이 두 벌이라
갈렸습니다. `propertyTitle` 하나로 모았습니다.

#### 같은 단지 매물이 지도에서 포개졌다

102동과 104동은 <b>좌표가 같았습니다.</b> 주소로 찍은 값이라 단지 대표점 하나뿐이고,
핀이 정확히 겹쳐 <b>뒤엣것은 누를 수도 없었습니다.</b>

겹친 것들을 작은 원(반지름 15m 남짓)으로 벌려 놓습니다. 순서는 동·호 이름으로
정합니다 — 정렬을 바꿔도 <b>같은 핀이 같은 자리</b>에 있어야 합니다.

> **이때 "동의 좌표는 우리에게 없다"고 적었는데, 틀렸습니다.** [I268]을 보십시오 —
> 재 보지 않고 단정한 것이었습니다. 벌려 놓기는 이제 <b>마지막 수단</b>입니다.

---

### I266. 매물 위에 <b>단지</b>를 둔다 · **[확정 — 구현됨]** · [I257] 확장

지금까지 매물은 저마다 고유했고, 같은 단지의 102동과 104동은 <b>아무 관계도 없는
남</b>이었습니다. 그래서 국토부 실거래를 <b>매물마다</b> 받았습니다.

```
한화포레나정릉 102동  →  12개월치 조회
한화포레나정릉 104동  →  12개월치 조회   ← 같은 단지 같은 84.9㎡인데
한화포레나정릉 1503호 →  12개월치 조회
```

#### 실거래는 매물이 아니라 <b>단지와 평형</b>에 붙는다

```
complex (단지)  1 ── N  property (매물)
      │
      └── N  reference_transaction   ← (complex_id, area_m2) 로 찾는다
```

면적은 딱 맞지 않습니다 — 84.93 과 84.98 은 같은 평형입니다. <b>담아 둘 때와
찾을 때가 같은 허용 범위(±15%)</b>여야 합니다. 다르면 담아 놓고 못 찾습니다.

#### 무엇이 같은 단지인가

이름과 <b>동·번지</b>입니다. 이름만으로는 `래미안` 이 전국에 있고, 번지만으로는
한 번지에 단지가 둘인 곳이 있습니다. 이름 정규화는 [I230]의 `ComplexName` 을
<b>그대로</b> 씁니다 — 같은 일을 두 곳에서 다르게 하다 갈라진 것이 그것이었습니다.

주소를 못 읽으면 <b>이름만으로</b> 묶되 열쇠에 그 사실을 남깁니다. 나중에 주소가
채워지면 다른 단지로 갈라져 새로 만들어집니다 — <b>섞이는 것보다 낫습니다.</b>

#### 그룹으로 가르지 않는다

아파트 단지도 국토부 실거래도 <b>공개 정보</b>입니다. 격리는 여전히
`property.group_id` 가 합니다 ([I87]) — 사람은 늘 <b>자기 매물을 통해서만</b>
단지에 닿습니다.

#### `Property` 레코드에는 넣지 않았다

이미 55칸이라 한 칸을 더하면 <b>25곳</b>을 고쳐야 하는데, 실거래를 찾는 데
필요한 것은 <b>이름과 주소로 만드는 열쇠</b>뿐입니다. 관계는 `property.complex_id`
열로 남깁니다 — 사람이 SQL 로 볼 수 있어야 하고, 그것이 이 요청의 본뜻입니다.

#### 옮기기는 SQL 로 하지 않는다

단지 열쇠는 `ComplexName.normalize` 가 만듭니다 — 괄호 안을 버리고 `아파트`를
지웁니다. SQL 로 옮겨 쓰면 <b>같은 규칙이 두 벌</b>이 되므로, 기동할 때 자바가
답니다(`ComplexBackfill`). 이미 달린 것은 건드리지 않아 여러 번 돌려도 같습니다.

옛 `reference_transaction` 행은 <b>버립니다.</b> 캐시라 다시 받으면 그만이고,
옮기려면 매물→단지 대응을 SQL 로 다시 만들어야 합니다.

#### 돌연변이 하나가 살아남았다

캐시 열쇠를 매물 번호로 되돌려도 테스트가 통과했습니다 — <b>DB 조회만으로도</b>
중복 호출이 막히기 때문입니다. 캐시 열쇠가 값을 하는 자리는 <b>헛걸음</b>이었습니다:
맞는 거래가 없는 단지에 새 매물을 넣으면, 매물마다 기억하는 한 12개월을
<b>처음부터 다시</b> 훑습니다. 그것을 재는 시험을 따로 더했습니다.

> **알아낸 것이 무엇의 성질인가**를 물어야 합니다. "이 단지엔 맞는 거래가 없다"는
> <b>단지의 성질</b>이지 매물의 성질이 아닙니다.

---

### I268. 동의 좌표는 <b>있었다</b> · **[확정 — 구현됨]** · [I265] 수정

[I265]에서 "동의 실제 위치를 아는 척하지 않습니다. 우리는 그 좌표를 갖고 있지
않습니다"라고 적었습니다. **재 보지 않고 단정한 것입니다.** 실제로 물어보니 있었습니다.

```
[address] 서울 성북구 정릉동 1037         37.60163325, 127.01085810
[address] 서울 성북구 정릉동 1037 102동    37.60163325, 127.01085810   ← 동을 무시한다
[keyword] 한화포레나정릉 102동            37.60146161, 127.01087812
[keyword] 한화포레나정릉 104동            37.60153734, 127.01037873   ← 약 45m 떨어져 있다
```

**주소검색과 장소검색이 다릅니다.** 우리는 주소검색만 쓰고 있었고, 그것은 동을
파싱해도 <b>지번 좌표 하나</b>를 줍니다. 장소검색에는 <b>건물마다</b> 있습니다.

#### 첫 결과를 그냥 쓰면 남의 단지에 찍는다

```
석관래미안 101동  →  래미안석관아파트 101동      37.60878, 127.05504
                  →  래미안아트리치아파트 101동   37.60654, 127.06220   ← 680m 떨어진 남
```

동 번호는 단지마다 겹칩니다. 첫 결과를 쓰면 <b>엉뚱한 곳에 핀이 찍히고,
그 잘못은 화면에 드러나지 않습니다.</b> 두 가지로 막습니다.

- 단지 좌표를 중심으로 <b>300m 안에서만</b> 찾는다 (한 단지 안의 동은 50m 안팎이었다)
- 돌아온 이름에 <b>그 동 번호가 실제로 들어 있어야</b> 받는다

#### 호는 버려야 한다

```
한화포레나정릉 102동          →  나온다
한화포레나정릉 102동 1503호    →  (결과 없음)
```

지도에 있는 것은 <b>건물</b>이지 세대가 아닙니다. `BuildingNumber` 가 동만 뽑습니다.

#### 못 찾으면 단지 좌표를 그대로 쓴다

키가 없거나, 카카오에 그 동이 없거나(`상계주공7단지 102동` 은 없습니다 — 7xx동입니다),
이름이 안 맞으면 <b>단지 좌표</b>입니다. 지어낸 자리로 밀어 놓는 것보다
겹쳐 있는 편이 낫고, 겹친 것은 화면이 벌려서 보여 줍니다([I265]).

#### 이미 넣어 둔 매물도 고친다

새로 등록하는 것만 고치면 <b>이미 있는 것은 영영 포개진 채</b>입니다.
기동할 때 <b>좌표가 같은 매물이 둘 이상인 자리</b>만 골라 물어봅니다 —
한 번 갈라지면 다음부터는 대상이 아니라 <b>여러 번 돌려도 같습니다.</b>
표시용 열이 필요 없습니다.

> **재 보고 말해야 합니다.** "그 데이터는 없다"는 확인할 수 있는 주장이었고,
> 확인하는 데 든 것은 API 호출 다섯 번이었습니다.

---

### I269. 매물 수정에서 <b>동/호를 고칠 수 있게</b> 한다 · **[확정 — 구현됨]** · [I265]·[I268] 후속

동/호는 수정 폼에 <b>칸이 없었습니다.</b> `carry` 에 담겨 <b>손대지 않고 되돌려보내는
값</b>이었습니다([I113]) — 잘못 넣으면 고칠 길이 없었습니다.

#### 칸을 만들면 `carry` 에서 빼야 한다

[I160]에서 겪은 그대로입니다. **양쪽에 두면 `carry` 가 폼 값을 덮어써서
고쳐도 안 바뀝니다.**

#### 동을 고쳤는데 핀이 안 움직였다

수정 폼은 좌표 칸을 <b>늘 채워</b> 보냅니다. `resolveCoordinates` 는 좌표가 오면
그대로 쓰므로, 102동을 104동으로 고쳐도 자리가 그대로였습니다 —
[I268]이 등록에만 듣고 있었습니다.

다시 찾아보는 것은 <b>사람이 좌표를 안 건드렸을 때뿐</b>입니다. 손으로 찍어 놓은
자리를 동 이름 때문에 옮겨 버리면 안 됩니다.

#### 이름이 갈린 자리가 <b>아홉 곳</b> 더 있었다

[I265]에서 목록·지도·상세를 `propertyTitle` 로 모았는데, <b>모달 제목 여덟과
임장 플래너 목록</b>이 남아 있었습니다. 같은 단지 매물을 둘 열면 채점·대출·코멘트
창이 <b>똑같은 제목</b>으로 떴습니다.

`x-text` 식에서 매물 이름을 <b>직접 꺼내 쓰는 자리</b>가 있는지 보는 검사를 뒀습니다.
`propertyTitle(x.property)` 는 `.name` 이 없어 안 걸립니다.

#### 같은 누락을 세 번째 겪었다

| | 무엇이 | 어디서 걸렸나 |
|---|---|---|
| [I265] | `buildPasteRequest` 가 `dongHo` 를 안 담았다 | <b>사람이 DB를 보고</b> |
| [I269] | 하마터면 `saveProperty` 가 안 담을 뻔했다 | 돌연변이 시험 |

빌드 단계가 없어 <b>아무 데서도 안 걸립니다.</b> `emptyPropertyForm` 의 칸이
모두 `saveProperty` 의 요청에 실리는지 보는 검사를 뒀습니다. 지금 보내지 않는 것은
셋뿐이고, 셋 다 그러는 것이 맞습니다 — `id`(URL)·`editVersion`(헤더)·`carry`(폼에 칸이
없는 값들).

> **칸을 고쳤는데 안 바뀌면 사람은 서버 탓을 합니다.** 화면과 요청이 어긋난 것을
> 화면만 보고는 알 수 없습니다.

---

### I283. 붙여넣기가 <b>한 줄로 붙어 오면</b> 일곱 항목이 빠졌다 · **[확정 — 실물로 드러난 버그]** · [I159] 후속

실제 붙여넣기 하나에서 단지명·동/호·KB시세·층·중개보수·지하철·학교가 통째로 빠지고,
<b>거래유형이 매매인데 전세로</b> 들어갔습니다.
...
#### 라벨을 `equals` 로만 찾고 있었다

가장 크게 문 것은 이것입니다. 네이버는 접기/펼치기 <b>버튼 글자를 라벨과 같은 줄에</b>
붙여 보냅니다 — `배정 초등학교상세내용 숨기기`. 라벨을 `equals` 로 찾으니 이런 줄은
<b>영영 안 걸리고</b> 그 블록이 통째로 빠졌습니다. 붙어 오는 화면 문구(`상세내용 숨기기`
· `도움말 보기` 등)만 떼고 봅니다. 학교·학교 도보시간이 이걸로 살아났고,
취득세·재산세도 덤으로 따라왔습니다.

#### 나머지

- **제목 가르기** — 동/호를 줄 <b>끝</b>에서만 찾아, 뒤에 뭐라도 붙으면 단지명이 줄
  전체가 되고 동/호는 빠졌습니다. <b>거래유형을 경계로</b> 잘라 그 앞을 "단지명 [동]"
  으로 읽습니다.
- **거래유형** — 본문을 훑어 첫 `매매|전세` 를 집었는데, 아래쪽 <b>토글 글자</b>
  ("매매 / 전세 / 월세")가 먼저 걸려 전세가 됐습니다. 제목을 먼저 믿고, 없으면
  `매매가`·`전세가` 라벨을 봅니다. <b>매매와 전세는 순위표가 아예 다르므로</b>
  (AGENTS.md) 조용히 틀리면 매물이 딴 표에 실립니다.
- **층** — 해당층이 숫자가 아니라 <b>밴드</b>로 옵니다(`고/19층`). 채점은 이미 밴드를
  다루는데 파서만 숫자를 고집했습니다.
- **KB시세** — 같은 줄에 붙어 온 값 뒤에 군더더기가 따라옵니다
  (`7억 3,000만원투기과열LTV 40%`). 붙어 오는 자리에서만 <b>앞부분 금액</b>을 읽습니다.
  줄 전체가 금액일 때만 읽는 `toWon` 은 그대로 뒀습니다 — 느슨하게 바꾸면 아무 문장에서나
  숫자를 집습니다.
- **중개보수** — `^최대` 가 줄 앞 들여쓰기에 막혔습니다.
- **지하철** — 라벨 다음이 <b>빈 줄</b>이라 빠졌습니다. 역 이름만 모아 `/` 로 잇습니다
  (`회기역/외대앞역`) — 도보시간은 `subwayMinutes` 가 따로 가지므로 이름에 안 섞습니다.

> **`valueAfter` 의 빈 줄 건너뛰기는 되돌렸습니다.** 문서에는 "다음 비어 있지 않은
> 라인"이라 적혀 있어 그렇게 고쳤는데, <b>어떤 시험도 그 변화를 잡지 못했습니다</b> —
> 필요 없었던 것입니다. 건너뛰며 찾으면 <b>다음 절의 첫 줄</b>을 그 라벨의 값으로
> 집을 수 있어, 없어도 되는 위험을 두지 않았습니다. 문서를 코드에 맞췄습니다.

고친 여섯 가지는 모두 <b>되돌리면 시험이 잡는지</b> 확인했습니다.

#### 표본 하나로는 모자랐다

다른 매물을 붙여넣자 <b>지하철만 또 빠졌습니다.</b> 같은 화면인데 붙여넣기 모양이
달랐습니다 — 들여쓰기가 없고, 역 이름에 <b>노선이 공백 없이</b> 붙어 옵니다.

```
회기역                ← 첫 표본
마들역7호선            ← 두 번째 표본: 노선이 붙는다
노원역4호선7호선        ← 여럿 붙기도 한다
```

역 뒤에 공백이나 줄끝을 요구하던 것을 <b>역까지만 끊도록</b> 바꿨습니다. 탐욕 반복이라
`동대문역사문화공원역` 처럼 이름 안에 `역`이 또 있어도 통째로 가져옵니다.

> **모양이 하나가 아닙니다.** 붙여넣기는 사람이 어디를 긁었는지에 따라 달라지므로,
> 표본 하나에 맞춘 규칙은 다음 표본에서 또 깨집니다.

#### 매물이 아니라 <b>모양</b>을 지킨다

처음에는 매물마다 원문을 통째로 픽스처에 두고 시험을 하나씩 붙였습니다
(`…ParseTest`). 그렇게 두면 <b>그 매물을 위한 시험</b>이 되어, 이름만 보고는 무엇을
지키는지 알 수 없고 매물이 늘 때마다 시험도 함께 늘어납니다. 게다가 원문은
개인정보가 있어 저장소에 두지 않으므로([I83]) <b>다른 곳에서는 통째로 건너뜁니다</b> —
지키는 것이 없는 시험이 됩니다.

그래서 `NaverPasteVariantsTest` 하나로 모으고, <b>모양마다</b> 최소한으로 잘라
두었습니다 — "제목이 한 줄로 붙어 올 때", "층이 밴드로 올 때", "라벨에 버튼 글자가
붙을 때", "역에 노선이 붙을 때". 단지 이름은 지어낸 것을 씁니다. 어디서 돌려도
건너뛰지 않고, 무엇을 지키는 시험인지 이름에 적혀 있습니다.

---

### I284. 끝내 못 잰 매물이 <b>며칠이고 「분석 중」</b>이었다 · **[확정 — 실물로 드러난 버그]** · [I220] 후속

카드에 「분석 중」 표시가 <b>멈추지 않고 돌았습니다.</b> 보정이 실패해 점수가 끝내
안 채워진 매물이었습니다 (그날은 Anthropic 키가 만료돼 401이 났습니다).

`scoring()` 은 <b>"점수가 비었으면 분석 중"</b> 이었습니다. 채워질 때까지만 참인 값이
아니라, <b>못 채우면 영원히 참인 값</b>이었던 것입니다. 판 번호 감시([I85])는 3초마다
계속 묻고, 화면은 계속 돌았습니다.

등록한 지 <b>5분</b>이 지나도 비어 있으면 표시를 거둡니다 — 보정·AI는 길어야 수십 초라
그보다 오래됐는데 비었다면 <b>기다리는 중이 아니라 못 낸 것</b>입니다. 언제 등록됐는지는
서버가 준 `createdAt` 으로 보므로 <b>새로고침해도 되살아나지 않습니다.</b> 등록 시각을
모를 때는 예전처럼 기다립니다 — 모른다고 조용히 감추지 않습니다.

#### 못 잰 것을 나쁜 점수처럼 칠하지 않는다

표시를 거두면 그 자리에 보통 점수 알약이 옵니다. 그런데 색이
`totalScore >= 50 ? 높음 : 낮음` 이라, 점수가 <b>없는</b> 매물이 붉은 「낮음」으로
칠해졌습니다 — [I220]이 "0점으로 보여 주면 안 된다"고 못 박은 바로 그 혼동입니다.
점수가 없으면 무채색(`is-unscored`)으로 `-` 만 보여 줍니다.

> **끝나지 않는 진행 표시는 없느니만 못합니다.** 돌고 있는 그림은 "곧 된다"는 약속인데,
> 지킬 수 없는 약속이면 사람은 화면을 못 믿게 됩니다.

---

### I285. 판 번호가 안 올라 화면이 <b>옛것을 붙들었다</b> · **[확정 — 실물로 드러난 버그]** · [I85] 후속

[I284]로 「분석 중」이 영영 도는 것은 막았지만, <b>왜 안 끝나는지</b>는 그대로였습니다.
배포 뒤에 두 가지가 더 드러났습니다.

#### 전망이 끝나도 아무도 안 알려 줬다

카드가 「분석 중」 표시(◌)에서 <b>화살표로 바뀌지 않았습니다.</b> 서버는 멀쩡히
`direction=FLAT` 을 주고 있었습니다 — 화면이 등록 당시의 `running: true` 를 그대로
들고 있었을 뿐입니다.

목록은 <b>판 번호가 달라질 때만</b> 다시 받습니다([I85]). 그런데 번호를 올리는 곳은
채점뿐이었고, <b>전망은 어디에서도 올리지 않았습니다.</b> 전망은 LLM 때문에 채점보다
한참 뒤에 끝나므로(운영 로그에서 20초), 그사이 번호는 이미 제자리로 돌아와 있고
화면은 다시 받을 이유를 영영 못 찾습니다.

전망이 끝나는 자리에서 번호를 올립니다. <b>저장했든, 그대로였든, 실패했든</b>
`running` 표시가 풀린 것은 같으므로 `finally` 에서 올립니다.

#### 번호를 커밋 전에 올리고 있었다

채점은 번호를 올리기는 했는데 <b>트랜잭션 안에서</b> 올렸습니다. 번호는 Redis 에,
점수는 DB 에 있습니다.

```
점수 기록(아직 커밋 전) → 번호 올림(Redis, 즉시 보임) → 커밋
                                    ↑ 이 틈에 화면이 읽으면
                          "번호는 새것인데 점수는 비어 있는" 판을 붙든다
```

붙들고 나면 <b>번호가 다시 바뀔 일이 없어 영영 못 빠져나옵니다.</b> 틈은 좁지만 한 번
걸리면 되돌아오지 않으므로 좁다고 넘길 수 없습니다. [I239]에서 캐시 무효화에 쓴 처방을
그대로 씁니다 — <b>커밋한 뒤에</b> 올립니다.

키와 올리는 방법은 `ScoreVersionPublisher` 한 곳에 뒀습니다. 부르는 곳이 둘(채점·전망)로
늘었는데 `"score:" + id` 를 양쪽에 적으면 언젠가 한 곳이 어긋납니다.

#### 화면에도 안전망을 뒀다

번호만 믿는 구조는 <b>한 번 어긋나면 회복 경로가 없습니다.</b> 그래서 「분석 중」인
카드가 남아 있는 동안에는 번호가 같아도 목록을 다시 받습니다. 기다리는 카드가 있을
때만, 그것도 [I284]의 유예 안에서만 그렇게 하므로 평상시에는 예전과 같습니다.

> **고칠 곳은 서버인데 증상은 화면에 납니다.** 신호가 없으면 화면은 <b>기다리는 것과
> 끝난 것을 구분할 방법이 없습니다</b> — 안 알려 주고서 화면 탓을 할 수는 없습니다.

---

### I286. 층은 숫자만이 아니다 · **[확정 — 실물로 드러난 버그]** · [I5] 후속

`3/21` 처럼 숫자로 적힌 층은 잘 되는데 <b>`저/15` 는 파싱도 저장도 되지 않았습니다.</b>
적어 넣어도 저장되지 않았습니다.

네이버는 저층 매물의 층수를 감춰 <b>밴드</b>(저·중·고)로 줍니다. 흔한 표기입니다.

#### 채점은 이미 밴드를 다루고 있었다

`FloorScorer` 는 처음부터 밴드를 봅니다 — 저=0, 중·고=동점 만점(AGENTS.md).
`Property` 에도 `floorBand` 칸이 있고 DB 에도 `floor_band` 가 있었습니다.
<b>비어 있던 것은 그 칸을 채우는 길이었습니다.</b>

- 화면의 층 칸이 `type="number"` 라 `저` 를 <b>적을 수가 없었습니다.</b>
- 화면은 `floorNo`(숫자)만 보냈고, `toNum('저')` 는 `null` 이라 <b>조용히 버려졌습니다.</b>
- `floorBand` 를 채우는 곳이 <b>어디에도 없었습니다.</b> 수정 화면은 그 값을 손대지 않고
  돌려보내기만 했습니다([I113]의 `carry`).

#### 적힌 그대로 보내고, 가르는 것은 서버가 한다

화면은 층을 <b>글자 그대로</b>(`floorRaw`) 보냅니다. 숫자면 `floorNo` 로, 저·중·고면
`floorBand` 로 나눠 담는 것은 `FloorValue` 한 곳에서 합니다 — 등록·수정·붙여넣기가
각자 가르면 언젠가 한 곳이 어긋납니다.

- 입력칸은 `text` 로 바꾸고 <b>숫자와 저·중·고만</b> 남깁니다. `저3` 처럼 섞으면
  뜻이 없으므로 글자 하나만 남깁니다.
- 수정 화면은 밴드로 저장된 매물의 칸에 <b>`저` 를 되살립니다.</b> 숫자만 되살리면
  빈 칸으로 보이고, 그대로 저장하면 <b>있던 값이 지워집니다.</b>
- 목록·상세는 `저/15층` 으로 보여 줍니다.

`floorRaw` 는 원래 붙여넣기에서 본 글자를 담던 자리라 `중층` 처럼 `층` 이 붙어 오기도
합니다. 그것도 읽습니다 — 화면 입력은 좁혔지만 <b>예전에 담긴 값을 못 읽으면 멀쩡한
층이 사라집니다.</b> 뜻을 알 수 없는 글자는 되돌려보냅니다.

> **칸은 있는데 채우는 길이 없었습니다.** 채점기도 DB도 밴드를 알고 있었으니
> "지원하지 않는다"고 말할 수도 없는 상태였습니다 — <b>절반만 만들어 둔 기능</b>이
> 조용히 값을 버리고 있었습니다.

---

### I289. 모바일 지도 위에 목록을 겹친다 · **[확정 — 구현됨]** · 11장 구체화

모바일의 지도·리스트 탭은 둘 중 하나를 완전히 감췄습니다. 지도에서 위치를 보다가 매물 정보를
확인하려면 화면을 바꿔야 하고, 다시 지도에 돌아오면 공간 관계를 이어 읽기 어려웠습니다.

모바일 본문은 <b>지도를 계속 유지</b>하고 목록을 그 위의 Bottom Sheet로 올립니다.

- 기본 높이는 헤더를 뺀 본문의 25%, 확장하면 본문 전체입니다.
- 손잡이는 Pointer Event로 위·아래 밀기를 받고 48px 이상 움직이면 다음 상태에 붙습니다.
- 손잡이를 누르거나 키보드 Enter·Space로도 두 상태를 오갈 수 있습니다.
- 목록 자체의 세로 스크롤은 확장 상태에서만 받습니다. 손잡이에만 `touch-action: none`을 두어
  지도와 목록의 기본 터치까지 빼앗지 않습니다.
- 마커를 누르면 목록을 펼친 뒤 해당 카드를 보여 줍니다.
- 목록에서 매물명을 누르면 지도 중심을 그 매물로 옮기고 시트를 25%로 접어 위치를 보여 줍니다.
- 지도 컨테이너는 시트를 움직일 때 크기가 바뀌지 않습니다. 화면 회전·브라우저 크기 변경처럼
  컨테이너가 실제로 바뀔 때만 `map.relayout()`을 부릅니다.

헤더는 49px 한 줄로 고정합니다. 모바일에서 다 들어가지 않는 관리·그룹·프로필·로그아웃은
더보기 메뉴에 두고, 매물·임장만 첫 줄에 남깁니다.

매물 카드 첫 줄에는 줄어들지 않는 배지가 여럿 있어 화면 밖으로 밀렸습니다. 이름·가격은 첫 줄,
등록자·그룹·임장 여부는 접히는 다음 줄로 나누고 카드와 목록의 `min-width`를 0으로 둡니다.
가로 스크롤을 가리는 데서 끝내지 않고 <b>넘치게 만든 고정 너비 조합</b>을 없앱니다.

---

### I290. 헤더는 모달을 덮지 않는다 · **[확정 — 구현됨]** · [I289] 후속

모바일 고정 헤더를 목록 시트보다 위에 두면서 `z-index: 50`을 줬더니, 자식 모달의 바탕인
매물 상세(`45`)보다도 위가 됐습니다. 상세가 화면을 덮어야 하는데 헤더가 그 위를 가렸습니다.

레이어 순서를 <b>본문·시트(10) → 헤더(40) → 매물 상세(45) → 일반 모달(50) → 자식 모달(60)
→ 확인 대화상자(70)</b>로 고정합니다. 헤더의 더보기 메뉴는 시트 위에 남지만 어떤 모달도
가리지 않습니다.

---

### I291. 모바일 카드 묶음은 PC 배치를 바꾸지 않는다 · **[확정 — 구현됨]** · [I289] 후속

[I289]에서 이름·가격과 배지를 모바일용 두 묶음으로 나눴더니 그 묶음이 PC에서도 flex 항목이
되어, 예측 화살표·가격·등록자 배지의 위치가 달라졌습니다.

`768px` 이상에서는 두 묶음을 `display: contents`로 풀어 기존 카드와 같은 한 줄로 놓습니다.
가격만 마지막 순서로 보내 <b>매물명 → 예측 화살표 → 등록자·그룹·임장 배지 → 가격</b>을
유지합니다. `767px` 이하에서는 묶음을 그대로 사용하므로 모바일의 두 줄 카드와 가로 넘침
방지는 바뀌지 않습니다.

---

### I292. 아이폰 HEIC는 JPEG로 바꾼 뒤 원본을 남기지 않는다 · **[확정 — 구현됨]** · [I63] 구체화

아이폰 사진은 HEIC이거나 1MB가 넘는 JPEG인 경우가 많습니다. Java 표준 ImageIO는 HEIC를
읽지 못하고 Spring의 기본 파일당 상한은 1MB라, 기존 업로드는 컨트롤러에 닿기 전 또는
Thumbnailator 디코딩 단계에서 실패했습니다.

- 파일당 업로드 상한은 기본 20MB, 요청 상한은 25MB로 명시하며 환경변수로 조절합니다.
- HEIC 여부는 확장자나 브라우저의 Content-Type이 아니라 ISO BMFF `ftyp` 내용으로 판별합니다.
- Java 25 ImageIO에 NightMonkeys HEIF 플러그인을 붙이고, 실행 환경에는 libheif 1.16 이상과
  `--enable-native-access=ALL-UNNAMED`를 둡니다.
- HEIC는 multipart 요청에서만 읽습니다. 앱 이미지 디렉터리에는 원본 HEIC를 쓰지 않고,
  최대 1920px JPEG와 320px JPEG 썸네일만 저장합니다. 요청 임시 파일은 요청 종료와 함께
  서블릿 컨테이너가 제거합니다.
- JPEG·PNG 등 기존 이미지의 EXIF 방향 보정 흐름은 유지합니다.
- 디코더가 설치되지 않은 경우와 손상된 이미지, 용량 초과를 서로 다른 응답으로 알리고
  매물 ID·크기·Content-Type·원인을 경고 로그에 남깁니다.
- 변환이나 DB 저장이 실패하면 이번 요청에서 만든 JPEG 파일도 지워 반쪽 파일을 남기지 않습니다.

---

### I293. 사진 모달을 닫아도 모바일 상세는 모바일 배치를 유지한다 · **[확정 — 구현됨]** · [I289] 후속

모바일에서 사진이 등록된 매물은 사진 목록의 최소 너비가 상세 그리드와 모달까지 밀어 PC처럼
넓어지는 경우가 있었습니다. 상세 섹션과 가로 사진 목록에 `min-width: 0`·`max-width: 100%`를
두어 사진 수와 무관하게 모달 안에서만 가로로 넘치게 하고, 모바일 상세 격자도
`minmax(0, 1fr)`로 내용보다 좁아질 수 있게 합니다.

---

### I294. 신혼 생활 시작 예산 계획은 그룹 자산·매물·혼수를 한 계획으로 묶는다 · **[확정 — PoC]**

주택 매물만 비교하던 Halley에 신혼부부의 주택·대출·혼수·자산을 함께 계산하는 `예산`
화면을 추가합니다. 화면은 별도 앱이 아니라 기존 `index.mustache`와 Alpine.js에 둡니다.

- 헤더의 `임장` 오른쪽에 `예산` 메뉴와 `/budget` 화면을 둡니다.
- 계획·자산·혼수·금융 데이터는 `group_id`로 격리합니다. 같은 그룹 구성원의 자산만 합산하며,
  기존 `users.available_budget`의 가격 채점 의미는 바꾸지 않습니다.
- 기존 등록 매물은 선택 modal에서 고르고, 가격·면적·거래유형을 계획에 복사한 뒤 사용자가
  수정할 수 있습니다. 매물을 고르지 않는 수기 입력도 허용합니다.
- 혼수 35개 seed 품목은 필수·추천·전체 시나리오와 사용자 정의 선택을 지원합니다. 품목별
  보유 여부, 선택 여부, 예산을 따로 저장해 보유품을 제외할 수 있습니다.
- 주택 자기자금·부대비용·혼수·투입 가능 자산·지원금을 결정론적으로 계산하고, 대출 월 상환액은
  금융기관 확정 심사가 아닌 참고용 추정치로 표시합니다.

혼수 품목에는 후보 상품과 대안 상품을 각각 상품명·URL·예상 금액으로 저장합니다. 사용자가
`상품 정보 읽기`를 누르면 서버가 허용된 도메인의 상품 페이지에서 JSON-LD·Open Graph·`title`
순서로 상품명과 가격을 미리 채우며, 사용자가 확인·수정한 뒤 저장합니다. 읽기 실패는 정상적인
수기 입력 경로로 전환합니다.

상품 URL preview는 임의 URL 프록시가 아닙니다. `http/https`만 허용하고, localhost·사설 IP·
링크 로컬 주소·내부 DNS·검증되지 않은 리다이렉트를 차단합니다. 응답 크기·시간·리다이렉트
횟수를 제한하며 원문 HTML은 저장하지 않습니다. 사이트 차단 우회나 네이버 매물 크롤링에는
사용하지 않습니다.

이 결정으로 추가하는 외부 연동은 상품 정보 preview뿐이며, 구현 전에 허용 도메인과 운영
정책을 설정값으로 확정합니다. 실거래가·KB시세·금융상품·자산 API는 PoC 범위에서 제외하고
후속 port/adapter 확장 대상으로 남깁니다.

### I295. 예산 LLM은 절감·구매 우선순위를 설명할 뿐 계산값을 결정하지 않는다 · **[확정 — PoC]**

예산 화면은 선택 매물의 전용면적·호가, 규칙 기반 예산 요약, 선택·보유 상태의 혼수 목록을
LLM에 보내 절감 후보와 구매 우선순위를 설명받을 수 있다. LLM은 다음만 출력한다.

- 입주 전 필수, 입주 후 구매 가능, 보유품 대체·제외 후보
- 면적 대비 과다한 가구·가전 구성 경고
- 사용자 입력을 바꾸지 않는 절감 방향과 근거

`BudgetCalculator`가 계산한 초기 필요자금·부족/잔여 자금·월 주거비는 확정값이며, LLM은
이를 계산하거나 수정·확정 대출 판단을 해서는 안 된다. 다른 그룹 자료·원문 HTML·매물
붙여넣기 텍스트·민감 개인 정보는 프롬프트에 포함하지 않는다. LLM 장애나 미설정은 예산
계산과 저장을 막지 않으며, 화면은 제안 없음으로 처리한다. PoC에서는 기존 AI 추천도 모델
설정을 재사용하고 결과는 요청 시 생성하며 영속하지 않는다.

### I296. 예산 부대비용은 내부 규칙으로 자동 추정하되 사용자가 확정한다 · **[확정 — PoC]**

법무사·등기, 이사, 입주청소처럼 사용자가 처음부터 알기 어려운 비용은 외부 견적 API에 의존하지 않고
버전 관리되는 내부 추정 규칙으로 제안한다. 매매가·전용면적·대출금·거래유형을 입력으로 사용하며, 취득세와
중개보수도 함께 참고값을 낸다.

- `POST /api/budget/plans/{id}/cost-estimate`는 저장하지 않는 추정값·계산 근거·주의사항을 반환한다.
- 적용은 사용자의 명시적 동작으로만 하며, 적용한 금액마다 `AUTO_ESTIMATE` 출처를 저장한다.
- 사용자가 특정 비용 입력값을 수정하면 그 항목만 `MANUAL` 출처로 바뀐다. 자동값과 수기값을 혼동하지 않는다.
- 취득세·중개보수는 주택 수, 규제지역, 지자체 조례, 협의·부가세 등 입력하지 않은 조건에 따라 달라질 수 있다.
  그러므로 PoC 값은 확정 세액·수수료·견적이 아니며 계약 전 관할 기관·중개사·법무사·이사업체에 확인한다.
- 외부 실거래가·금융상품 API는 이 비용 추정의 입력이나 근거로 사용하지 않는다. 향후 견적 제휴가 생기면
  별도 port/adapter와 출처를 추가한다.

### I297. 금융결제원 오픈뱅킹 잔액 연동은 연결 계좌의 금융자산 보조 입력으로만 쓴다 · **[설계 — 구현 전]**

금융결제원 오픈뱅킹 잔액조회는 사용자가 OAuth와 계좌등록으로 연결한 계좌의 잔액을 가져오는 데
적합하다. 따라서 예산의 금융 자산 후보를 채우되, 자동으로 투입 가능 금액을 확정하거나 수기 자산을
덮어쓰지 않는다. 부동산·자동차·증권 시가·미연결 계좌를 망라하는 전체 자산 조회 수단으로는 쓰지 않는다.

구현은 이용기관 승인·callback URL 등록·최신 운영 명세 확인 후에만 시작한다. 토큰과 핀테크이용번호는
사용자별로 암호화해 PostgreSQL에 저장하고, 로그·Slack·LLM·브라우저 저장소에 남기지 않는다. API 호출은
OpenFeign client와 FallbackFactory로 분리한다. 상세 설계는 `docs/KFTC_OPEN_BANKING_ASSET_SYNC_DESIGN.md`에 둔다.
...
```
송산 58.59㎡ · 345세대 · 기준 2026-09
  최근 3개월 (2026-06~08) : 4건  ✅
  직전 3개월 (2026-03~05) : 2건  ❌  ← 여기서 끊겼다
```

**작은 단지는 늘 판단 보류였습니다.** 345세대 단지의 한 평형은 1년에 8건쯤
팔립니다 — 3개월 창에 3건은 채우기 어렵습니다.

표본 기준을 낮추면 <b>2건짜리 중앙값</b>을 추세라 부르게 되어 [I130]의 취지가
무너집니다. **기준 대신 기간을 늘립니다** — 3개월로 안 되면 6개월로 넓혀 다시 셉니다.

- 넓혔다는 사실은 <b>근거 문장에 적힙니다</b> — 3개월 값인 줄 알면 오해합니다
- 3개월로 되면 <b>굳이 넓히지 않습니다.</b> 좁을수록 최근을 잘 비춥니다
- 넓혀도 안 되면 <b>내지 않습니다</b>

> **[I147]이 지키려던 것은 그대로입니다.** 그것은 "창을 <b>위치</b>로 자르지 말라"는
> 것이었지 "넓히지 말라"가 아니었습니다. 지금도 창은 <b>달력</b>으로 자릅니다 —
> 빠진 달은 0건으로 셀 뿐, 더 오래된 달이 그 자리를 메우지 않습니다.

---

### I253. 실거래 지표가 <b>왜</b> 빠졌는지 말한다 · **[확정 — 구현됨]**

이유가 넷인데 <b>화면도 로그도 아무 말이 없었습니다.</b>

| 어디서 끊겼나 | 문구 |
|---|---|
| 자료를 못 받음 | `이 지역의 실거래 자료를 받아 두지 못해…` |
| 단지명 불일치 | `실거래 412건을 받았지만 '송산'과 이름이 맞는 거래가 없어…` |
| 평형 불일치 | `412건 중 이름이 맞는 것은 31건이지만 전용 58.59㎡와 맞는 평형이 없어…` |
| 거래가 흩어짐 | `이름·면적이 맞는 실거래는 14건이지만 비교 구간마다 3건을 채우지 못해…` |

> 마지막 문구를 처음에 <b>"14건뿐이라"</b>로 썼습니다. **14건이 있는데 틀린 말입니다** —
> 총량이 아니라 <b>구간마다</b> 안 찬 것입니다.

**이 문장은 LLM 에도 갑니다.** 이유를 안 알려 주면 모델이 "인근 실거래 비교 자료가
없습니다" 같은 <b>지어낸 추측</b>을 씁니다 — 실제로 그랬습니다.
`※` 로 시작하는 줄은 지표가 아니라 메모임을 시스템 프롬프트가 못 박습니다.

---

### I254. 분석이 끝나면 결과를 연다 · **[확정 — 구현됨]** · [I146] 조정

매매가를 눌러 2분을 기다렸는데 <b>화면이 그대로</b>였습니다.

```js
if (found?.forecast?.direction === 'UNCERTAIN' && this.noModalOpen())
```

[I146]에서 <b>판단 보류일 때만</b> 열게 해 두었습니다 — 방향이 나오면 화살표가 곧
응답이라는 뜻이었습니다. 그런데 [I248]에서 판단 보류를 `FLAT` 으로 바꾸면서
이 조건이 <b>아무것도 안 걸리게</b> 됐습니다.

**끝나면 엽니다.** 눌러서 시킨 일이라 결과를 보고 싶은 것이 당연하고,
화살표만으로는 왜 그렇게 나왔는지 알 수 없습니다. 그 사이에 사람이 다른 것을
열었으면 덮지 않습니다.

---

### I255. 5년의 <b>모양</b>을 보낸다 · **[확정 — 구현됨]**

LLM 에 가는 것은 <b>점 몇 개</b>였습니다 — 4년 전 12개월 중앙값, 최근 12개월 중앙값,
5년 전고점, 최근 3개월. **그 사이가 어떻게 움직였는지는 안 갔습니다.**
...
```
[연도별 실거래 중앙값]  ※ 방향을 세는 지표가 아니라 흐름을 보는 자료다
2022년 5.95억 (12건) · 2023년 6.4억 (9건) · 2024년 7.4억 (11건) · …
```

- 숫자 다섯 개라 <b>토큰 부담이 거의 없습니다</b>([I149]에서 답이 잘린 적이 있습니다)
- 건수를 함께 적어 <b>두 건짜리 중앙값을 추세로 읽지 않게</b> 합니다
- 해가 셋에 못 미치면 <b>안 냅니다.</b> 없는 모양을 지어내지 않습니다

#### 지표로 만들지 않았다

처음에 `PriceIndicator` 로 만들었다가 되돌렸습니다. **지표로 내면 `FLAT` 한 표가
되어 판단 보류를 늘립니다** — [I248]에서 "유지가 과반이면 무조건 유지"로 정했기
때문입니다. 같은 움직임을 실거래 추세·장기 추세가 이미 세고 있어,
...
### I257. 주소로 단지를 가린다 · **[확정 — 구현됨]** · `COMPLEX_NAME_MATCHING.md` ①

이름이 <b>통째로 바뀐</b> 단지가 있습니다.
...
대우 `푸르지오` → 한화 `꿈에그린` → `포레나` 로 이어진 단지라, 국토부에 옛 이름으로
남아 있으면 <b>부분 문자열 비교로는 영원히</b> 안 맞습니다.

#### 이미 오고 있던 것을 버리고 있었다

국토부 응답에 <b>법정동과 번지가 옵니다</b>(`umdNm`·`jibun`). 파싱조차 안 하고
있었습니다.
...
**번지 불일치를 배제 근거로 쓰지 않습니다.** 번지가 여러 개인 대단지가 있어서,
다르다고 곧바로 빼면 <b>멀쩡한 거래를 잃습니다.</b> 번지 일치는
<b>이름을 이기는 근거로만</b> 씁니다.

#### 저장된 주소를 먼저 봤다

```
경기도 하남시 망월동 938        서울시 성북구 안암동3가 138
서울시 노원구 상계동 692        서울시 노원구 공릉동 81
서울시 성북구 석관동 407        서울시 성북구 정릉동 1037
                               서울시 성북구 동소문동4가 279
```

**일곱 건 전부 번지가 있었습니다.** 없었으면 ①보다 ③(사람이 고르기)이
먼저였습니다 — 재료를 확인하지 않고 순서를 정할 뻔했습니다.

#### `안암동3가` 가 `안암동` 이 되면 안 된다

기존 정규식(`\S+?(?:동|읍|면)`)은 <b>`안암동` 까지만</b> 끊습니다. 국토부는
`안암동3가` 로 주므로 그렇게 끊으면 영영 안 맞습니다.

공백으로 잘라 <b>마지막 시·군·구 다음 토막</b>을 통째로 씁니다 —
`서울시 노원구` 처럼 둘이면 뒤엣것이 기준입니다.

#### 규칙은 `ComplexMatch` 하나다

전망과 실거래 카드가 <b>이것만</b> 부릅니다. [I230]에서 같은 규칙이 두 벌이라
전망이 늘 자료 부족이었습니다.

> 그때 `ComplexName` 으로 통일하며 <b>호출부만 바꾸고 함수는 남겨</b> 두었습니다 —
> `ReferenceTransactionService.normalizeComplexName()` 은 아무도 안 부르는 채로
> 남아 있었습니다. 지웠습니다.

#### `jibun` 이 없으면 `bonbun`·`bubun` 으로

네 자리 0채움이라 그대로 쓰면 `0138` 이 됩니다. 숫자로 읽어 되짚습니다.
