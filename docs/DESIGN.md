...
> **그래서 값을 자동 반영하지 않고 감지만 합니다.** 규제를 잘못 읽으면 한도가 통째로
> 틀어지는데 알아챌 방법이 없습니다. `발령일자`가 바뀌면 Slack으로 알리고 `제개정이유`의
> 지역 목록을 함께 실어, **사람이 읽고 관리 화면에 입력**합니다. 판단은 사람이 하되
> 놓치지는 않는 구조입니다.

`OC` 파라미터는 이메일 ID입니다. `test`는 시험용이므로 운영에서는 발급받아 씁니다.
