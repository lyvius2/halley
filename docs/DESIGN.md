...
### I98. 실거래 조회를 24개월로 넓힌다 · **[확정 — 실물로 드러난 문제]**
...
`ministry.reference.lookback-months`(기본 24)만큼 **달마다 한 번씩** 부릅니다.
**이번 달은 건너뜁니다** — 신고 지연 때문에 물어 봐야 빈 응답입니다.

매물당 한 번 돌고 결과는 저장되므로(캐시) 등록 시점의 비용입니다. 등록 직후 비동기 보정에서
이미 트리거됩니다(I53).

> **비었을 때 얼마나 찾아본 것인지 화면에 말합니다** — `최근 24개월 동안 같은 단지·면적의
...
보정은 외부 API를 60번 가까이 줄지어 부르는데(공시가격 페이징 최대 15회, 실거래 24개월치,
