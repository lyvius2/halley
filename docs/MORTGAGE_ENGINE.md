...
>
> **상태: 로드맵 1·2·4단계 구현 완료** (담보가치 우선순위 · 방공제 · 실거래 단가 보정 ·
> 규제지역·보유주택 LTV 분기 · 규제 관리 화면). 3·5단계는 미구현입니다(6장).
...
- 규제지역은 `regulated_area` 테이블(법정동코드 기준)로 두고 관리자가 관리합니다.
  ...
  고시 변경 감지는 법제처 API로 가능합니다(I73, 미구현).
...
| 저장 | `CachePort` (TTL 1일). 상품 목록은 DB에 둘 만큼 오래 쓰지 않는다 |
