...
>
> **상태: 설계 단계.** 3장의 절반은 이미 구현되어 있고, 나머지는 미구현입니다(6장 로드맵).
...
| 방공제(최우선변제금) | **미구현** | ⚠️ 아래 2.3 — 가장 큰 오차 원인 |
...
| 단계 | 내용 | 규모 |
|---|---|---|
| **1** | 방공제·담보가치 우선순위 반영 (2.1·2.3) | 작음 — 기존 `LoanCalculator` 확장 |
...
**1단계만으로도 현재 산출값의 정확도가 눈에 띄게 올라갑니다.** 방공제가 빠져 있는 것이
지금 가장 큰 오차이고, 외부 연동 없이 고칠 수 있습니다.
