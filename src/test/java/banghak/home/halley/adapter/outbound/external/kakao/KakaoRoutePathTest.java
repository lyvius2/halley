 ...
 */
@DisplayName("카카오 경로선 파싱 (설계 I177)")
    ...
    void readsKakaoVertexes() {
        final DriveRoute route = adapter.parse("""
                {"routes":[{"summary":{"duration":1980,"distance":11125},
                 "sections":[{"roads":[
                   {"name":"신림로","vertexes":[126.929713,37.484191,126.929712,37.484254]},
                   {"name":"남부순환로","vertexes":[127.027618,37.497949]}]}]}]}
                """);

        assertThat(route.durationMinutes()).isEqualTo(33);
        assertThat(route.path().points()).hasSize(3);
        // 서울 안이어야 한다 — 뒤집히면 위도 126, 경도 37 이 되어 소말리아 앞바다다
        final RoutePath.Point first = route.path().points().getFirst();
        assertThat(first.lat()).isBetween(37.0, 38.0);
        assertThat(first.lng()).isBetween(126.0, 128.0);
        // 도로가 여러 개면 이어 붙인다
        assertThat(route.path().points().getLast().lat()).isEqualTo(37.497949);
    }
