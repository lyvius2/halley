 ...
 * 지표를 세어 방향을 정한다.
 *
 * 결론은 지표에서 나옵니다. LLM 이 스스로 낸 방향은 쓰지 않습니다 —
 * LLM 은 지표와 근거만 제공하고 판정은 우리가 합니다.
 *
 * 유지 = 판단 보류
 *
 * 둘을 같은 것으로 봅니다. `유지`는 "올라가지도 내려가지도 않는다"고 단언하는
 * 말이 아니라 "방향을 말하지 않겠다"는 뜻입니다. 실제로 파서가
 * LLM 의 "모르겠다"를 `FLAT` 으로 바꿔 넣고 있어(), `FLAT` 은 이미
 * 모름의 하치장입니다.
 *
 * 규칙
 *
 ...
 *
 *
 * 우선순위는 상승 &gt; 유지 &gt; 하락 입니다. 같은 수면 위쪽이 이깁니다 —
 * ③과 ④가 그것입니다.
 *
 * 적극적으로 읽습니다. 유지가 과반이 아니면 방향을 말합니다.
 * 지표 넷 중 상승 둘·하락 둘이면 상승입니다.
 */
public record FactorTally(int up, int flat, int down) {
     ...
     * 셀 것이 없다.
     *
     * 지표가 하나도 없거나 전부 유지인 경우입니다. 방향으로는 둘 다 `유지`
     * 지만, 화면은 다르게 보여 줍니다(🤔) — "재료를 보고 판단을 안 한 것"과
