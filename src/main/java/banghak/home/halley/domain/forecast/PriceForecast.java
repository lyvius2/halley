 ...
 * 저장된 가격 전망.
 *
 * @param llmDirection  LLM 이 스스로 낸 결론. outlook.direction() 은
 *                      규칙까지 거친 최종 결론이라 둘이 다를 수 있습니다 —
 *                      LLM 이 판단을 보류해 우리가 지표를 세어 넣었으면 여기는 UNCERTAIN
 *                      입니다. 누가 무엇을 말했는지 남겨야 '유력'을 가릴 수 있고
 *                      사후 검증(구현 10)에서 누가 맞혔는지 셀 수 있습니다.
 *                      옛 행은 null 입니다 — "모른다"는 뜻이라 유력이 안 붙습니다
 * @param codeDirection 코드 예측. 결론이 아니라 참고 문구용입니다.
 *                      나중에 실제 결과와 맞춰 어느 쪽이 더 맞았는지 보려면 이 값이 필요합니다
 * @param promptHash    같은 지표면 다시 묻지 않기 위한 값
...
public record PriceForecast(
     ...
     * 유력한가.
     *
     * LLM 이 방향을 말했고(상승·하락), 지표를 세어도 같은 쪽이면
     * 유력입니다. 두 갈래가 따로 같은 결론에 닿은 것이라 더 믿을 만합니다.
     *
     * LLM 이 판단을 보류해 우리가 세어 넣은 경우는 유력이 아닙니다.
     * 견줄 상대가 없었으니까요 — 그때 direction 과 지표 판정은 같을 수밖에
     * 없습니다. 그것을 "일치"로 읽으면 늘 유력이 됩니다.
     */
