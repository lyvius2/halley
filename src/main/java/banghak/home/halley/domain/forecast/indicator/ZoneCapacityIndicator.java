 ...
 * 용도지역과 재건축 여력.
 *
 * 용도지역은 재건축 사업성의 뿌리입니다. 제1종일반주거(상한 150%)와
 * 제3종일반주거(300%)는 같은 단지라도 이야기가 완전히 다릅니다.
 *
 * 건축물대장(I132)이 붙어 현재 용적률을 실측으로 압니다.
 * `여유 = 조례 상한 − 현재 용적률`.
 *
 * 연식과 함께 봅니다
 *
 * 신축에 용적률 여유가 크다고 재건축 호재가 아닙니다. 실측(동탄역시범호반써밋)에서
 * 2015년 준공에 여유가 127%p였는데, 재건축은 수십 년 뒤 이야기입니다 —
 * "여유를 안 쓰고 지었다"는 뜻일 뿐입니다.
 *
 * 그래서 연식이 기준에 못 미치면 여유를 말하지 않고 용도지역만 씁니다.
 * 안 그러면 신축 단지마다 "재건축 여력 큼"이 뜹니다.
 *
 * 대장을 못 받았을 때도 마찬가지입니다 — 근사값으로 채우지 않습니다.
 */
public class ZoneCapacityIndicator implements PriceIndicator {
    ...
    /**
     * @param farLimits             용도지역명 → 용적률 상한(비율). 지자체 조례라 지역마다
     *                              다릅니다 — `regulation_param`에서 주입합니다
     ...
     * 재건축 여력.
     *
     * 연식이 안 찼으면 내지 않습니다. 신축의 용적률 여유는 재건축과 무관합니다.
     *
     * @return 상한을 모르거나, 대장이 없거나, 연식이 모자라면 null
     */
     ...
     * 실제 적용되는 용도지역 하나.
     *
     * `INCLUDED`만 봅니다. 저촉·접함은 옆 필지의 것이라
    ...
    private String evidence(LandUse zone, BigDecimal limit, BuildingLedger ledger, BigDecimal headroom) {
        if (limit == null) {
            // 상한을 모르면 모른다고 씁니다. 임의의 값을 넣지 않습니다
