 ...
 * 규칙으로 방향을 낸다.
 *
 * 이것은 결론이 아닙니다. 결론은 LLM이 냅니다(4.5). 이 예측은
 * 눈가림 2차 소견으로, 화면에서 "AI와 규칙 계산이 갈렸습니다"를 말하는 데 씁니다.
 * LLM에게는 넘기지 않습니다 — 보여 주면 모델이 끌려가 두 예측이 독립이 아니게 됩니다.
 *
 * 합산하지 않습니다
 *
 * 요인을 점수로 합치려면 가중치가 필요한데 그 근거가 없습니다. 합치는 순간
 * 임의의 숫자가 객관적 예측처럼 보입니다. 대신 세어서 정합니다.
 *
 *
 ...
 *
 *
 * 무게(HIGH·MEDIUM·LOW)는 표의 개수로만 씁니다 — 실거래 추세 한 표가
 * 용도지역 한 표보다 무겁다는 정도의 순서만 담습니다. 소수점 가중치를 두지 않는 이유는
...
public class RuleBasedForecaster {
    ...
    /**
     * @return 요인이 하나도 없으면 UNCERTAIN. 억지로 방향을 고르지 않습니다.
     */
     ...
     * 요인들이 얼마나 한 방향인가.
     *
     * 방향이 갈리는데 확신이 높을 수는 없습니다. 표가 한쪽으로 몰릴수록 높습니다.
     */
     ...
     * 이 판단이 놓치고 있는 것.
     *
     * 비워 두지 않습니다. 재료가 없는 것은 늘 있고, 그것을 말하지 않으면
