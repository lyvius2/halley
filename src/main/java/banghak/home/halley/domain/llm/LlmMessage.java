 ...
 * LLM에 보내는 한 번의 요청.
 *
 * 공급자마다 파라미터 이름이 다르므로(Claude는 `system`이 최상위, Ollama는 메시지 배열의 한 역할)
 * 포트에서는 공통분모만 담고 변환은 어댑터가 맡습니다.
 *
 ...
 *                  호출자가 고를 수 있어야 한다
 * @param temperature 답의 흔들림. null이면 보내지 않고 공급자 기본값에 맡깁니다 —
 *                  기존 호출(AI 추천도·비교 우위)의 동작을 바꾸지 않으려는 것입니다.
 *                  판단하는 작업은 0을 씁니다.
 *                  다만 요즘 Claude 모델은 이 값을 받지 않습니다 —
 *                  어댑터가 기본으로 빼고 보냅니다. 뜻은 남겨 둡니다: 받는 모델이나
 *                  다른 공급자로 내려갈 때 켜면 됩니다. 같은 지표에 같은 답은
