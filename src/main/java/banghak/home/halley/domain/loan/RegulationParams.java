 ...
 * 대출 산정에 쓰는 규제 수치.
 *
 * 모든 값은 `regulation_param` 테이블의 활성 프로파일에서 옵니다. 규제가 바뀌면 새 프로파일을
 ...
 * @param stressApplyRatio   스트레스 DSR 단계 적용률. 규제가 단계적으로 올라와
...
public record RegulationParams(
     ...
     * 실제로 더할 스트레스 금리.
     *
     * 기준 금리에 단계 적용률과 금리유형 가중치를 곱합니다. 고정금리는 가중치가 0이라
