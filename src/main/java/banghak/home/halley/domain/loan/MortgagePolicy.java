 ...
 * LTV 비율을 규정으로 판정한다.
 *
 * Rule Engine은 외부를 부르지 않습니다. 규제 파라미터와 판정 재료를 받아 비율만 냅니다.
 * 순수 함수여야 규정이 바뀌었을 때 테스트로 확인할 수 있습니다.
 *
 * 비율은 `ltv.rate.{지역}.{보유}` 형태의 파라미터에서 옵니다. 값이 없으면 프로파일의
 * 기본 비율(`ltv.rate`)로 떨어집니다 — 규정을 모르는 상태에서 0%로 막아 버리면
 * 화면이 고장난 것처럼 보입니다.
 *
 * 생애최초는 지역·보유와 무관하게 우대 비율을 적용하되, 별도의 총액 상한이 붙습니다.
 */
