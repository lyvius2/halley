 ...
 * 지도에 그릴 실제 경로선.
 *
 * 처음에는 점 하나의 목록이었습니다. 그러면 경로 전체가 한 색이라
 * 어디서 갈아타는지, 어디가 막히는지를 지도에서 알 수 없습니다.
 *
 * 이제 구간(segment) 목록입니다. 구간마다 색을 달리 칠합니다 —
...
public record RoutePath(List<Segment> segments) {
     ...
     * 한 가지 색으로 그릴 구간.
     *
     * @param style 색을 고를 열쇠. 색을 여기서 정하지 않습니다 —
     *              색은 화면의 것이고, 서버는 무엇인지만 말합니다
