 ...
 * 사람이 적어 넣은 층.
 *
 * 층은 숫자만이 아닙니다. 네이버는 저층 매물의 층수를 감춰 `저/15층` 처럼
 * 밴드로 줍니다. 채점은 이미 밴드를 다루는데(`FloorScorer` — 저=0, 중·고=만점)
 * 들어오는 길이 숫자뿐이라, 밴드로 적힌 매물은 파싱도 저장도 되지 않았습니다.
 *
 * 적힌 것을 그대로 두고(raw), 숫자면 floorNo 로 밴드면
 * floorBand 로 나눕니다. 둘 다 되는 값은 없습니다.
 *
 * 가르는 규칙을 여기 한 곳에 둡니다 — 등록·수정·붙여넣기가 각자 가르면
 * 언젠가 한 곳이 어긋납니다.
 */
public record FloorValue(String raw, Integer floorNo, FloorBand band) {
     ...
     * 예외를 던지지 않습니다. 파싱 실패는 MISSING 으로 남기는 것이 이 프로젝트의
     * 규칙입니다(AGENTS.md).
     *
     * `층` 이 붙어 와도 읽습니다. 이 칸은 원래 붙여넣기에서 본 글자를 그대로
     * 담던 자리라 `중층` · `3층` 처럼 옵니다. 화면 입력은 숫자와 저·중·고 로 좁혔지만,
     * 예전에 담긴 값과 다른 경로로 들어오는 값까지 못 읽으면 멀쩡한 층이 사라집니다.
     */
