 ...
 * 전망을 다시 낸다.
 *
 * 등록 때 한 번 내고 끝이었습니다. 실거래는 매달 새로 들어오는데 화살표는
 * 등록 시점 그대로였습니다 — 반년 지난 매물의 화살표가 반년 전 시장을 가리킵니다.
 *
 * 사후 검증(구현 10)에도 이것이 필요합니다. 재계산이 없으면 이력에
 * 매물당 한 건만 쌓여 견줄 과거가 생기지 않습니다.
 *
 * 매월 15일에 돕니다. 국토부 신고 기한이 계약 후 30일이라 월초에는
 * 지난달 자료가 덜 찹니다(I129의 신고 지연과 같은 이유). 04:15 —
 * 규제지역(04:00)·공시금리(04:30)와 시간을 벌립니다.
 *
 * 대개 값이 쌉니다. 60개월 중 새로 받을 달은 서너 달뿐이고(I128),
...
public class PriceForecastJob {
     ...
     * 판매완료·작성 중은 건너뛴다.
     *
     * 이미 팔린 매물의 전망은 아무 판단에도 안 쓰입니다. 그런데도 돌리면
     * 매달 국토부 호출만 늘어납니다.
     */
