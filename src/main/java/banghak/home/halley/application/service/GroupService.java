 ...
 * 그룹 가입·이동·정리.
 *
 * 초대 코드로만 그룹을 옮깁니다(규칙 3·8). 코드 전달은 앱이 하지 않습니다(규칙 10).
 */
@Slf4j
@Service
public class GroupService {
     ...
     * 그룹 정보 화면용 상세.
     *
     * 구성원과 매물 수까지 한 번에 담습니다. 화면이 값마다 따로 부르면
     ...
     *
     * 그룹의 누구나 바꿉니다 — 이름 변경과 같은 기준입니다. 비우면 알림이 나가지
     * 않습니다. 전역 주소로 흘려보내면 우리 매물이 남의 채널에 뜹니다.
     */
    @Transactional
     ...
     *
     * 주소를 잘못 넣어도 알림이 조용히 안 갈 뿐이라, 넣고 바로 확인할 수 있어야 합니다.
     */
     ...
     *
     * 코드가 기본키라 겹치면 삽입이 실패합니다. 그때 다시 뽑습니다 — 만들기 전에
     ...
     *
     * 원래 그룹에 아무도 남지 않으면 그 그룹과 매물이 함께 사라집니다(규칙 4).
     ...
     *
     * 매물은 그룹에 딸리므로 그룹이 사라지면 볼 사람이 없습니다. 남겨 두면 아무도 못
     * 보는 자료가 계속 쌓입니다.
     */
    @Transactional
     ...
     *
     * 이름을 비우면 무작위 한국어로 짓습니다. 이름은 겹칠 수 없습니다 — 같은 이름이
     * 둘이면 회원을 넣을 때 어느 쪽인지 화면에서 가릴 수 없습니다.
     *
     * 회원이 없는 그룹은 {@value #MAX_EMPTY_GROUPS}개까지입니다. 빈 그룹은 아무
