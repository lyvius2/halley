 ...
 * 매물이 규제지역에 속하는지 판정한다.
 *
 * 판정 키는 법정동코드입니다. `property.pnu`(I54)의 앞 10자리가 곧 법정동코드이므로
 * 외부를 부르지 않고 씁니다. pnu가 없으면 지번주소로 시군구 코드를 역매핑합니다.
 *
 * 지정 정보가 없으면 {@link RegulationZone#NORMAL}입니다. 규제지역인데 등록을 안 해 둔 경우
 * 한도가 실제보다 높게 나오지만, 반대로 기본값을 규제로 두면 전국이 규제지역이 되어
 * 훨씬 크게 틀립니다.
 */
@Slf4j
@Service
