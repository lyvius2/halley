 ...
 *
 * AI 추천도는 매물 제원만 보고 매기는 값이 아니라 사용자들이 이 집을 어떻게 보는지도
 * 재료로 씁니다. 쾌적함 점수와 코멘트가 그 재료이므로, 바뀌면 다시 물어야 합니다.
 *
 * 화면에서 '다시 물어보기' 버튼을 없앤 자리를 이 이벤트가 대신합니다. 사용자가 언제 눌러야
 * 하는지 알기 어려웠고, 누르지 않으면 옛 판단이 그대로 남았습니다.
 *
 * @param kind   무엇이 바뀌었는지. 재질의 트리거이자 어떤 알림을 보낼지 가르는 값이다
 * @param detail 무엇으로 바뀌었는지 — 쾌적함이면 점수, 코멘트면 그 글.
...
public record PropertyInsightChanged(Long propertyId, Kind kind, String actorNickname,
     ...
     * 매물 제원이 바뀌었다.
     *
     * 사람의 판단이 아니라 매물 자체가 바뀐 경우입니다. 면적·층·가격·주차 같은
     * 값이 프롬프트에 그대로 실리므로, 고쳤으면 다시 물어야 옛 판단이 남지 않습니다.
     *
     * 알림은 보내지 않습니다 — 등록·삭제와 달리 수정은 함께 보는 사람에게 알릴 일이
