 ...
 * 범용 캐시.
 *
 *에 하나의 `CachePort` 로 적혀 있던 것입니다. 구현할 때
 * 타입마다 쪼개져 여덟 개가 됐고, 그 어긋남을 나중 문서가 "계열"이라 부르며 정당화했습니다(I164).
 * 캐시 구현체가 달라도 동일한 인터페이스를 사용한다.
 *
 * 키와 TTL 을 부르는 쪽이 정합니다. `PropertyDetailCache`(I158)는 키를 매물 번호로,
 * TTL 을 24시간으로 못 박아 두어 매물에 딸리지 않은 것이나 수명이 다른 것을
 * 담을 수 없었습니다.
 *
 * JSON 문자열로 담습니다. 담을 것이 응답 형태(DTO)라 포트에 그 타입을 끌어들이면
...
public interface CachePort {
     ...
     * "찾아봤지만 없었다".
     *
     * 결과가 아니라 이미 찾아봤다는 사실을 담습니다. 실거래는 못 찾으면
     * 저장할 것이 없어, 저장된 게 없으면 상세를 열 때마다 12개월치를 다시
     * 받아 왔습니다.
     */
     ...
     * "지금 누가 받아 오고 있다".
     *
     * 에서 화면이 3초마다 다시 묻게 했는데, 물을 때마다 배경 조회를
     * 새로 띄웠습니다 — 1분이면 스무 벌입니다. 표시가 있으면 기다리기만 합니다.
     *
     * TTL은 끝났는데 표시가 남는 것을 막는 안전장치입니다. 정상 종료 때는
     ...
     * "이 매물은 지금 보정 중이다".
     *
     * 등록 응답을 기다리지 않고 돌려주므로, 목록·상세가 아직 점수가 없는
     * 매물을 만납니다. 그때 그 자리에서 채점해 버리면 기다림이 옮겨 갔을 뿐
     * 입니다 — 표시가 있으면 "곧 채워진다"고 답하고 넘어갑니다.
     *
     * TTL 을 둡니다. 보정 중에 서버가 죽으면 표시만 남는데, 그러면 그 매물은
     * 영영 채점되지 않습니다.
     */
     ...
     * 바뀌는 지점이 명확해서 무효화가 쉽다는 것이 더 중요합니다.
     */
     ...
     * 관련 기사 검색 결과. 키는 검색어다.
     *
     * 매물 번호가 아닙니다 — 같은 단지의 매물 둘은 검색어가 같고, 그러면
     * 한 번만 물으면 됩니다.
     */
    String NEWS = "news";
     ...
     * 자가용 길 하나.
     *
     * 열쇠는 좌표 넷과 출발 시각입니다. 시각을 빼면 화요일 14시와 일요일
     * 14시가 같은 길이 되어이 무의미해집니다.
     *
     * 매물 일곱이면 한 번 계산에 49쌍입니다. 담아 두지 않으면 「경로 계산」을
     ...
     * 그 갈래를 통째로 지운다.
     *
     * 중개사 정보를 고치면 그 중개사가 붙은 매물 전부가 낡습니다. 어느 매물인지
