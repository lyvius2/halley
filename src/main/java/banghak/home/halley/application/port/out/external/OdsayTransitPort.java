...
public interface OdsayTransitPort {
     ...
     *
     * 키가 없으면 조회가 조용히 빈 결과를 내는데, 부르는 쪽이 그것을 경로가 없는 것과
     * 구분하지 못했습니다. 직주근접이 왜 미산출인지 말해 주려면 이 구분이 필요합니다.
     *
     * 기본값을 둔 이유는 이 인터페이스를 람다로 대신하는 테스트가 여럿이기 때문입니다.
     * 실제 어댑터는 키를 보고 답합니다.
     */
     ...
     * 그 경로의 실제 선을 받아 온다.
     *
     * mapObj 는 `findTransit` 이 돌려준 열쇠입니다 — 그것 없이는 못 받습니다.
     * 그래서 호출이 한 번 더 듭니다. 지도에 그릴 때만 부릅니다.
     *
     * 기본값은 빈 경로입니다 — 이 인터페이스를 람다로 대신하는 테스트가 여럿이라
     * 메서드를 늘리면 그 전부가 깨집니다.
     */
     ...
     * 여러 구간을 한꺼번에.
     *
     * 임장 행렬은 매물 8개면 72쌍입니다. ODsay 라면 쌍마다 불러도 괜찮지만
     * (50ms), 할당량이 끝나 LLM 으로 넘어가면 쌍마다 부르는 것은 못 씁니다 —
     * 한 번 계산에 수십 분입니다.
     *
     * 기본 구현은 그냥 돕니다. 묶어서 이득을 보는 구현만 이것을 덮습니다.
     *
     * @param legs 열쇠 → {startX, startY, endX, endY}
     * @return 답을 낸 것만. 못 낸 열쇠는 빠집니다 — 빈 자리를 0이나 999로 채우지 않습니다
     */
