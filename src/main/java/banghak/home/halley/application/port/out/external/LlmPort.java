 ...
 * LLM 공급자 추상화.
 *
 * 지금 구현체는 Claude(`ClaudeLlmAdapter`) 하나지만, 나중에 Ollama 같은 로컬 모델을 붙일 수 있게
 * 공급자에 종속된 개념을 넣지 않습니다 — 프롬프트 캐싱·툴 호출·스트리밍은 여기 없습니다.
 * 여러 구현이 공존하면 `halley.llm.provider`로 고릅니다.
 *
 * 실패는 예외가 아니라 {@link LlmResult#failed(String)}로 돌려줍니다. LLM은 보조 입력이라
 * 죽어도 나머지 채점은 그대로 나와야 합니다.
 */
