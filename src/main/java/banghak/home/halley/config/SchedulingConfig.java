 ...
 * 스케줄러를 켜 둔다.
 *
 * 한때 매물 생존 확인 배치의 cron 을 `system_config` 에서 읽어 동적으로 걸었습니다.
 * 그 기능을 걷어냈습니다 — 네이버 부동산은 매물이 완매돼도
 * URL 이 정상 응답을 주므로, HTTP 상태로는 판별할 수 없었습니다.
 *
 * 남은 잡들은 전부 @Scheduled 로 각자 cron 을 답니다.
...
public class SchedulingConfig {
     ...
     * 커넥션 풀을 들여다볼 통로.
     *
     * DataSource 를 {@link ConnectionPoolWatch} 가 직접 받으면 생성자가
     ...
     * 스케줄러 스레드를 둘로.
     *
     * 기본값이 하나입니다. 잡이 여섯인데 하나로 돌면 먼저 잡은 것이 끝날 때까지
     * 나머지가 밀립니다 — 전망 배치처럼 오래 도는 것이 있어 실제로 밀립니다.
     *
     * 감시는 여기 안 태웠습니다({@link ConnectionPoolWatch} 는 제 스레드를 씁니다).
