 ...
 *
 *
 * 보정은 외부 API 대기가 대부분이라 커넥션을 잠깐만 쥡니다 — 아마 문제
 * 없습니다. 그런데 확인한 적이 없습니다. 마르면 그 보정은 3초 뒤 실패하고,
 * 실패한 보정은 로그만 남깁니다. 사용자 눈에는 "왜 얘만 채점이 안 됐지" 로
 * 보입니다.
 *
 * 왜 HikariCP 의 DEBUG 로그로 안 되는가
 *
 * com.zaxxer.hikari.pool.HikariPool 을 DEBUG 로 올리면 풀 상태를
 * 30초마다 찍습니다. 코드를 안 고쳐도 되니 처음엔 그것을 권했는데,
 * 보정 폭주는 몇 초짜리입니다. 두 샘플 사이에 일어나면 통째로 안 보이고,
 * 하루 내내 waiting=0 만 보고 "문제없다"고 잘못 결론냅니다.
 *
 * 1초마다 봅니다. connection-timeout 이 3초라 1초를 못 넘기는 대기는
 * 애초에 아무도 못 죽입니다 — 해가 될 수 있는 것은 전부 걸립니다.
 *
 * 왜 @Scheduled 가 아닌가
 *
 * 이 앱의 스케줄러는 스레드 하나입니다(spring.task.scheduling.pool.size
 * 기본값). 거기에 배치가 여섯 개 물려 있어, 전망 배치처럼 오래 도는 것이 잡고 있으면
 * 그동안 감시가 멈춥니다 — 하필 DB 가 바쁜 그때입니다.
 *
 * 그러면 30초 샘플링과 같은 잘못이 됩니다: 못 본 것을 없었던 것으로 읽습니다.
 * 감시는 제 스레드에서 돕니다. 가상 스레드 하나가 정수 셋을 읽고 자는 것이라
 * 값이 없습니다.
 *
 * 조용합니다
 *
 * 줄 서 있는 것이 없으면 아무것도 안 찍습니다. 평소 로그를 늘리지 않으려는
 * 것이고(), 그래서 켜 두고 잊어도 됩니다 — 찍혔다면 볼 것이 있다는 뜻입니다.
 *
 * 최고치는 기억해 둡니다. 폭주는 새벽에도 일어나고, 그때 사람이 보고
 * 있을 리 없습니다.
 */
@Slf4j
@Component
public class ConnectionPoolWatch {
     ...
     * 이만큼 줄 서면 말한다.
     *
     * 1~2는 정상입니다 — 커넥션이 오가는 중에 잠깐 겹치는 것뿐입니다.
     * 그것까지 찍으면 진짜 문제가 소음에 묻힙니다.
     */
     ...
     * 제 스레드에서 돈다.
     *
     * 데몬입니다 — 이것 때문에 앱이 안 죽는 일은 없어야 합니다.
     */
    @PostConstruct
    void start() {
        running = true;
     ...
     *
     * 0이면 안 찍습니다. "괜찮았다"는 말을 매일 남길 이유가 없습니다 —
     * 남아 있는 줄이 곧 무슨 일이 있었다는 뜻이어야 합니다.
     */
     ...
     *
     * 감시가 안 된다고 앱이 죽을 이유는 없습니다. 다만 그때는 보고 있다고
     * 착각해서도 안 되므로 기동 때 한 번 말합니다.
     */
