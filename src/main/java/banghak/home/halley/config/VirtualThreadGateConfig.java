 ...
 * 가상 스레드 게이트를 둘 둔다.
 *
 * 상한을 하나로 나눠 쓰면 60개월을 훑는 전망 하나가 다른 매물의 보정을 전부
 * 밀어냅니다. 가격 전망은 한 매물에 60번을 던지는데, 보정은 매물당 몇 번이라
 * 같은 줄에 세우면 뒤엣것이 하염없이 기다립니다.
 *
 * 전망 쪽 상한이 훨씬 작습니다(6 대 400). 공공 API에 60건을 한꺼번에 던지면
 * 429가 돌아옵니다 — 값이 싼 가상 스레드와 달리 그 끝에 붙은 API는 값이 비쌉니다.
 *
 * 동시 실행 상한만으로는 모자랐습니다. 국토부는 초당 요청 수를 세므로
 * {@link RateGate}를 따로 둡니다.
 */
@Configuration
public class VirtualThreadGateConfig {
     ...
     * 임장 경로 계산용.
     *
     * 매물 7개면 구간이 49쌍입니다. 예전에는 이것을 한 줄로 돌았습니다 —
     * 카카오 읽기 제한이 6초이니 최악이면 5분입니다. 프록시가 60초에 끊어
     * 504가 났습니다.
     *
     * 상한이 작습니다. 길찾기 API는 한꺼번에 몰면 429를 돌려줍니다 —
     ...
     * 국토부는 초당 요청 수를 셉니다.
     *
     * 동시 실행 상한만으로는 부족했습니다. 6으로 묶어도 각 호출이 40ms에 끝나면
     * 초당 150건이 나가고, 실제로 그렇게 나가서 전부 429를 받았습니다.
     *
     * 전망(60개월)과 실거래 카드(12개월)가 같은 키를 씁니다. 그래서 게이트도
