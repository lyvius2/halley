 ...
 *
 * 빈이 여럿입니다(`VirtualThreadGateConfig`). 보정용과 전망용의 상한이 다릅니다 —
 * 하나를 나눠 쓰면 60개월을 훑는 전망 하나가 다른 매물의 보정을 전부 밀어냅니다.
 *
 * 가상 스레드는 값이 싸서 수천 개도 만들 수 있지만, 그 끝에 붙은 외부 API는 그렇지 않습니다.
 * 매물을 여러 건 연달아 등록하면 카카오·V-World·국토부에 동시 요청이 몰려 429가 돌아옵니다.
 * 그래서 스레드가 아니라 동시 실행 수를 세마포어로 묶습니다 — 상한을 넘은 작업은
 * 거절되지 않고 자리가 날 때까지 기다립니다.
 *
 * 세마포어는 애플리케이션 전체가 하나를 나눠 씁니다. 매물마다 새로 만들면
...
public class VirtualThreadGate {
     ...
     *
     * 한 작업이 터져도 나머지는 그대로 끝냅니다 — 공시가격 조회가 실패했다고
     * 실거래가까지 날아갈 이유가 없습니다. 실패한 자리에는 null이 들어갑니다.
     */
     ...
     * 맡겨 놓고 기다리지 않는다.
     *
     * {@link #runAll} 은 이름 그대로 전부 끝날 때까지 붙잡습니다. 그런데
     *에서 "화면은 기다리지 않는다"고 써 놓고 그 함수를 불렀습니다 — 요청 하나가
     * 12개월치를 다 받을 때까지 붙잡혀 있다가, 그러고 나서 "받아 오는 중입니다"라고
     * 답했습니다. 기다리지 않으려면 기다리지 않는 함수가 있어야 합니다.
     *
     * 결과도 예외도 돌려주지 않습니다. 맡기는 쪽이 스스로 기록해야 합니다.
     */
     ...
     *
     * {@link #runAll} 은 끝날 때까지 기다립니다. 바깥 API가 느려지면
     * 그만큼 요청이 붙잡히고, 프록시가 60초에 끊으면 504가 됩니다 —
     * 임장 경로 계산에서 실제로 그랬습니다.
     *
     * 시간이 다 되면 남은 것을 버립니다. 무엇이 남았는지는 부르는 쪽이
     * 압니다 — 채워졌는지 보고 대체 수단으로 넘기면 됩니다.
     *
     * 버릴 때 shutdownNow 로 끊고 기다리지 않습니다. 기다리면
     * 시간을 정해 둔 뜻이 없어집니다.
     */
