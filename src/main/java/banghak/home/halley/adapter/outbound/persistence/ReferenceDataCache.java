 ...
 * 왜 저장소 안에 두는가
 *
 * 부르는 자리가 흩어져 있습니다 — criterion만 해도 9곳, system_config는
 * 12곳입니다. 부르는 쪽마다 캐시를 감싸면 어느 하나가 빠집니다. 더 나쁜 것은
 * 지우는 쪽입니다: 저장하는 자리에서 지우는 것을 한 군데라도 빠뜨리면
 * 그때부터 옛 값으로 계산합니다. 읽는 곳과 지우는 곳을 한 클래스 안에 두면
 * 둘이 어긋날 자리가 없습니다.
 *
 * 이 프로젝트에서 반복된 실패입니다 — 규칙이 두 벌, 진단이 두 벌,
 * 스타일이 두 벌.
 *
 * N+1을 먼저 걷어냈습니다
 *
 * 캐시는 증상을 가립니다. 캐시가 비는 순간(배포 직후·수명 만료·Redis 장애)
 * 원래 부하가 그대로 돌아옵니다. 그래서에서 매물마다 순위표를 읽던 것을
 * 먼저 고치고, 그 위에 이것을 얹습니다.
 *
 * 실패해도 값은 옵니다
 *
 * 캐시를 못 읽거나 못 쓰는 것은 느려질 뿐이지 기능이 죽을 일이 아닙니다.
 * 이 클래스를 쓸 때 운영의 Redis 는 실제로 죽어 있었습니다 — 그 상태에서도 채점과
 * 대출 계산은 돌았습니다. Redis 컨테이너를 내려도 마찬가지여야 합니다.
 *
 * DB 가 가까워져도 씁니다
 *
 * PostgreSQL·Redis 가 앱과 같은 EC2 안 Docker 로 오면서 왕복이 20ms 에서
 * 1ms 아래로 내려갔습니다. 그렇다고 걷어낼 이유는 되지 않습니다 — 왕복이
 * 싸졌다고 해서 사람이 안 건드리는 표를 요청마다 다시 물을 이유가 생기지는 않습니다.
 *
 * 다만 나중에 "느린 게 캐시 탓인가"를 물을 때 가를 수 있어야 하므로
 * halley.cache.reference.enabled 로 껐다 켤 수 있습니다. 기본은 켜짐이고,
 * 꺼도 답은 같습니다.
 */
@Slf4j
@Component
public class ReferenceDataCache {
     ...
     *
     * 끄는 것은 재거나 의심할 때뿐입니다 — "느린 게 캐시 탓인가"는
     * 켠 채로만 재면 가를 수 없습니다(ADJUST_CACHE.md §5.1).
     *
     * 꺼도 동작은 같습니다. 늘 원본에서 읽을 뿐입니다 — 무효화가 없으니
     * 낡은 값이 나올 일도 없습니다.
     *
     * 값은 application.yaml 에 ${HALLEY_CACHE_REFERENCE_ENABLED:true}
     * 로 적혀 있습니다. 여기 @Value 에만 두면 grep 으로 안 잡혀
     * 문서에만 있는 스위치가 됩니다 — 실제로 한 번 그랬습니다.
     */
     ...
     * 없을 수도 있는 것을 담는다.
     *
     * "없다"도 담습니다. 없을 때만 원본까지 가면 없는 것을 물을 때마다
     * 캐시가 아무 일도 안 합니다 — legal_dong_code 처럼 못 찾는 경우가 잦은
     * 사전에서 특히 그렇습니다(에서 실거래에 같은 처방을 했습니다).
     *
     * Optional 대신 빈 목록으로 담습니다 — 직렬화가 단순하고,
     ...
     * 바뀌었으니 지운다.
     *
     * 수명이 다하기를 기다리면 그동안 옛 값으로 계산합니다. 규제 파라미터가
     * 그러면 대출 한도가 틀립니다. 키 하나가 아니라 갈래를 통째로 버리는 이유는,
     * 어느 키가 낡았는지 되짚는 것보다 단순하고 기준 정보 수정은 드물기 때문입니다.
     *
     * 두 번 지웁니다
     *
     * 트랜잭션 안이라면 커밋한 뒤에 한 번 더 지웁니다. 쓰기 시점에만 지우면
     * 커밋 전에 끼어든 읽기가 옛 값을 다시 담아 버리고, 그러면 수명이 다할 때까지
     ...
     *
     * 갈래를 통째로 버리는 것({@link #evict})은 Redis 에서 SCAN 입니다.
