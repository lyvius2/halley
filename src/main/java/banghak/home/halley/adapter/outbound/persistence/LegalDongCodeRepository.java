...
public class LegalDongCodeRepository {
    ...
    public LegalDongCode save(LegalDongCode legalDongCode) {
        dsl.insertInto(TABLE)
                ...
                .execute();
        // 낡은 칸이 어느 것인지 정확히 안다 — 갈래를 통째로 훑을 이유가 없다.
     ...
     *
     * 못 찾은 것도 담습니다. 사전에 없는 동네가 드물지 않고, 그때마다
     * 원본까지 가면 캐시가 아무 일도 안 합니다 —에서 실거래에 쓴 처방과 같습니다.
     *
     * findAll 은 담지 않습니다. 수만 행이라 한 덩어리로 담을 것이 못 되고,
     ...
     * 채워져 있으면 기동할 때마다 V-World를 다시 부르지 않는다.
     */
