 ...
 * 한국은행 ECOS 어댑터.
 *
 * 통계 `121Y006`(예금은행 대출금리, 신규취급액 기준)에서 가계대출 항목만 골라 옵니다.
 * 한 통계 안에 대출평균·기업대출·가계대표 등 여러 항목이 섞여 오므로 `ITEM_CODE1`으로
 * 걸러야 합니다 — 안 거르면 기업대출 금리가 섞여 들어옵니다.
 *
 * 항목 코드를 프로퍼티로 둡니다. 한국은행이 코드 체계를 바꾸면 코드 수정 없이
 * 설정만 고쳐 대응합니다.
 */
@Slf4j
@Component
