...
/**
 * 규제지역 고시 첨부 PDF에서 지정 현황표를 읽는다.
 *
 * 추출된 텍스트가 깨져 있습니다. 한글 문서에서 나온 PDF라 숫자와 문장부호가 줄 끝으로
 * 밀립니다 — 강남구 서초구 송파구, , , 처럼 쉼표가 뒤로 몰립니다. 그래서 구분자를 믿지
 * 않고 부호를 모두 지운 뒤 공백으로 토큰화합니다. 지역명 토큰 자체는 순서까지 온전합니다.
 *
 * 표는 `시도 / 현행 / 조정` 3열이고 변동이 없는 시도는 `좌동`으로 적힙니다. `현행`과
