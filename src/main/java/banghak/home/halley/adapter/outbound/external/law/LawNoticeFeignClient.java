 ...
 * 법제처 국가법령정보 Open API.
 *
 * OC는 인증키가 아니라 이메일 ID입니다(`hong@korea.kr` → `hong`). 시험용
 * test로도 응답하지만 언제까지 열려 있을지 보장이 없어 운영에서는 발급받아 씁니다.
 *
 * 규제지역 고시는 법률이 아니라 행정규칙이라 target=admrul입니다.
 *
 * base-url은 https여야 합니다. http로 부르면 301로 https에 넘기는데, Feign이 쓰는
 * HttpURLConnection은 프로토콜이 바뀌는 리다이렉트를 따라가지 않습니다.
